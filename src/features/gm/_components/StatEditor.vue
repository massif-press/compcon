<template>
  <div class="px-2">
    ...
    >
      <v-row
        v-if="item.StatController.DisplayKeys.length"
        dense
      >
        <v-col
          v-for="kvp in displayKeys.filter(x => !hiddenKeys.includes(x.key) || x.added)"
            ...
            <v-card-text>
              {{
                $t('gm.stats.resetConfirm', {
                  ...
                  name: controller.Class ? controller.Class.Name : '',
                })
              }}
            </v-card-text>
            <cc-button
...
<script setup lang="ts">
  ...
  import { Bonus, tierValue } from '@/classes/components/feature/bonus/Bonus'
    ...
    return (props.bonuses as Bonus[])
      ...
      .map(b => {
        ...
        if (Array.isArray(b.Value)) {
          ...
          resolved = Number(
            props.item.FeatureController?.EvaluateSpecial(tierValue(b.Value, tier)) ?? 0
          )
        }
        if (resolved === null) return b
        const icon =
          b.Overwrite || b.Replace

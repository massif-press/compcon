                ...
                >
                  <v-col cols="9">
                    <v-combobox
                      ...
                      :error-messages="
                        labelExists(label) ? ['Label with this name and value already exists'] : []
                      "
                      density="compact"
                      item-value="title"
            ...
            >
              ...
              >
                ...
                <div style="position: absolute; top: 32px; bottom: 0">
                  ...
                  >
                    <v-chip
                      v-for="label in sortedAvailableLabels"
                      :key="label.title"
                      size="small"
                      color="accent"
...
<script setup lang="ts">
  ...
  const allLabels = computed(() =>
    uniqBy(
      [...NarrativeStore().getAllLabels, ...NpcStore().getAllLabels].filter(
  ...
  const availableLabels = computed(() =>
    allLabels.value.filter(
      (label: any) =>
        !(props.item as any).NarrativeController.Labels.some(
          (item: any) => item.title === label.title
        )
    )
  )
  ...
  const sortedAvailableLabels = computed(() => {
    ...
    else if (freqSortDir.value === 1)
      labels.sort(
        (a, b) => (labelFrequency.value[b.title] || 0) - (labelFrequency.value[a.title] || 0)
      )
    else if (freqSortDir.value === 2)
      labels.sort(
        (a, b) => (labelFrequency.value[a.title] || 0) - (labelFrequency.value[b.title] || 0)
      )
    return labels
  })
  ...
  watch(dialog, val => {
    if (!val) {
      ;(props.item as any).NarrativeController.Labels = (
        props.item as any
      ).NarrativeController.Labels.filter((label: any) => label.title.length > 0)
    }
  })
  ...
  function labelExists(label: any) {
    return (
      (props.item as any).NarrativeController.Labels.filter(

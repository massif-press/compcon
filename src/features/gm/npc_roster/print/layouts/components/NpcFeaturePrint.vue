<template>
  ...
  >
    ...
    <v-col
      v-if="isWeapon || feature.HasAccuracy || feature.HasAttackBonus"
      class="px-1 text-center"
    >
      <template v-if="isWeapon">
        <span
          v-for="(r, ri) in asWeapon.Range(tier)"
          :key="`range-${ri}`"
          class="pl-1"
        >
          <v-icon
        ...
        <cc-slashes
          v-if="asWeapon.Damage(tier).length"
          class="pl-2"
        />
        <span
          v-for="(d, di) in asWeapon.Damage(tier)"
          :key="`damage-${di}`"
          class="pl-1"
        >
          <v-icon
      ...
      >
        ...
        />
        <v-icon
          :icon="feature.Accuracy(tier) < 0 ? 'cc:difficulty' : 'cc:accuracy'"
          class="mt-n1"
        />
        {{ signed(feature.Accuracy(tier)) }}
      </span>
      <span
        v-if="isWeapon || feature.HasAttackBonus"
        class="pl-1"
      >
        <cc-slashes
          v-if="isWeapon || feature.HasAccuracy"
          class="pl-1 pr-2"
        />
        <v-icon
          icon="cc:reticle"
  ...
  <div v-if="(feature as NpcWeapon).Attacks && (feature as NpcWeapon).Attacks.some(x => x > 1)">
    <i18n-t
      keypath="gm.featurePrint.weaponAttacks"
      tag="span"
      scope="global"
    >
      <template #count>

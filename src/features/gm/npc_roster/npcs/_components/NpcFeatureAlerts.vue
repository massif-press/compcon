<template>
  <div v-if="!hide">
    <div v-if="expanded">
      <cc-alert
        v-for="item in templateController.FeatureRequirements.filter(
          item => !item.complete || !item.optional_complete
        )"
      ...
      >
        <div v-if="!item.optional_complete && item.optionalMin && !item.optionalMax">
          {{
            $t(
              'gm.npcAlert.selectUpTo',
              { n: item.optionalMin - item.selected },
              item.optionalMin - item.selected
            )
          }}
        </div>
        <div v-else-if="!item.optional_complete">
          {{
            $t(
              'gm.npcAlert.selectUpTo',
              { n: item.optionalMax - item.selected },
              item.optionalMax - item.selected
            )
          }}
        </div>
        <div v-else-if="!item.complete">
          {{ $t('gm.npcAlert.selectN', { n: item.max - item.selected }, item.max - item.selected) }}
        </div>
      </cc-alert>
    </div>
    <v-menu

...
<script setup lang="ts">
  ...
  const groupings = computed(() => {
    const allLabelTitles = new Set(
      npcStore.getAllLabels
        .filter((x: any) => x.title && x.title.length > 0)
        .map((x: any) => x.title)
    )
  ...
  function openItem(item) {
    selected.value = item
    if (mobile.value) viewRef.value.minimize()
  }

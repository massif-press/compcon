  ...
  async function importFile() {
    ...
    for (const item of staged) {
      try {
        ...
        notify({
          title: t('gm.import.importErrorTitle'),
          text: t('gm.import.encounterImportErrorText', { error: String(error) }),
          icon: 'cc:compendium',
          color: 'error',
        })
      }
    ...
    if (backedUp > 0) {
      notify({
        title: t('gm.import.v2BackupTitle'),
        text: t('gm.import.v2BackupText', { count: backedUp }, backedUp),

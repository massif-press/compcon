...
import { getColor, findNearest } from './mapUtils'
...
function init() {
  if (props.map) {
    mapData = props.map
    mapData.extentsX = mapData.SizeX
    mapData.extentsY = mapData.SizeX
  } else {
    mapData = new EncounterMap({ type: 'HexH', x: props.sizeX, y: props.sizeY, tiles: [] })
  }
  ...
  for (let row = 0; row < props.sizeY!; row++) {
    for (let col = 0; col < props.sizeX!; col++) {
      ...
      if (mapData.HasTile(col, row)) {
        ...
        if (flags.length > 2) { context.fillText('ALL', x + xOffset - 1, y - 2) }
        else if (flags.includes('Player')) { context.fillText('PC', x + xOffset + 1, y - 2) }
        else if (flags.includes('Enemy')) { context.fillText('NPC', x + xOffset - 1, y - 2) }
      } else {
...
async function HandleClick(event: MouseEvent) {
  ...
  const extentsX = props.sizeX! * cellSize * 3
  const extentsY = props.sizeY! * cellSize * (4 / 5)
  if (mouseX < 0.5 * -cellSize || mouseX > extentsX) return

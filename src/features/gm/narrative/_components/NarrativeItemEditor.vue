...
<script setup lang="ts">
  ...
  function exportItem(item: any) {
    if (props.itemType === 'Character') exportAsJson(Character.Serialize(item), `${item.Name}.json`)
    else if (props.itemType === 'Faction')
      exportAsJson(Faction.Serialize(item), `${item.Name}.json`)
    else exportAsJson(Location.Serialize(item), `${item.Name}.json`)
  }
</script>

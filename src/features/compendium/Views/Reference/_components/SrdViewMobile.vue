<template>
  <div style="container: size">
    <div
      class="heading"
      style="font-size: calc(25px + 2.5cqw); padding-left: 2.5vw"
    >
      {{ title }}
    </div>
  </div>
  <v-container
    :style="mobile ? 'padding:0' : 'padding: 0 12vw 0 12vw'"
    class="pb-12"
  >
    <fieldset class="mx-1">
      ...
      <v-card-text class="pa-0 my-2">
        <v-row justify="space-around">
          <v-col
            v-for="n in Math.ceil(content.length / 3)"
            :key="`col-${n}`"
          >
            <div
              v-for="(item, itemIdx) in content.slice(3 * (n - 1), 3 * n)"
              :key="`toc-item-${itemIdx}`"
            >
              <div

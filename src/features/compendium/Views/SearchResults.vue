      ...
      <template v-if="searchText.length >= 3">
        <v-row align="center">
          ...
          <v-col cols="auto">
            <i class="text-overline">
              {{
                $t(
                  'compendium.search.resultCount',
                  { count: compendiumResults.length },
        ...
        <v-row align="center">
          ...
          <v-col cols="auto">
            <i class="text-overline">
              {{
                $t('compendium.search.resultCount', { count: srdResults.length }, srdResults.length)
              }}
            </i>
          </v-col>
        </v-row>
        <v-container>
          <v-card
            v-for="(result, index) in srdResults"
            :key="`srd-${index}`"
...
<script setup lang="ts">
  ...
  watch(
    () => route.query.search,
    val => {
      searchText.value = Array.isArray(val) ? (val[0] ?? '') : ((val as string) ?? '')
    },
    { immediate: true }
  )
  ...
  const srdResults = computed(() => {
    ...
    return uniqBy(matched, 'title').sort((a, b) => {
      ...
      return (
        (b.content.match(searchRegex.value) ?? []).length -
        (a.content.match(searchRegex.value) ?? []).length
      )
    })
  })

<template>
  <cc-sidebar-view>
    ...
    <v-row no-gutters class="mt-3" align="center">
      ...
      <v-col v-if="$vuetify.display.smAndDown" cols="auto" class="ml-auto" style="max-width: 30%">
        <v-select
          v-model="manFilter"
          value="ALL"
          :items="['ALL', ...sources]"
    ...
    <div v-for="m in Object.keys(bonuses)">
      ...
      <cc-core-bonus-item v-for="b in bonuses[m]" :id="`e_${b.ID}`" :bonus="b" class="mx-3 my-5" />
    </div>
  </cc-sidebar-view>
</template>
...
export default {
  ...
  computed: {
    bonuses(): Dictionary<CoreBonus[]> {
      let bArr = CompendiumStore().CoreBonuses.filter((x: CoreBonus) => !x.IsHidden);
  ...
  methods: {
    ...
    manufacturer(id: string): any {
      return CompendiumStore().Manufacturers.find((x) => x.ID === id.toUpperCase());
    },
  },
};
</script>

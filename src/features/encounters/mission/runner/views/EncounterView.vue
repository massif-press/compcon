<template>
  <div>
    <v-row
      id="scroll"
      dense
      style="height: calc(100vh - 185px); overflow-y: scroll"
    >
      <v-col>
    ...
    <v-row v-if="actors" dense style="min-height: 100px">
      ...
      <v-col cols="1" class="text-center">
        ...
        <v-btn block tile color="primary" @click="stageRoundEnd()"
          >End Round</v-btn
        >
        ...
        <v-menu offset-x left>
          ...
          <v-card>
            <v-card-text class="text-center font-weight-bold">
              This will conclude the encounter and progress the mission. This
              can not be undone. Are you sure you want to continue?
              <v-divider class="my-2" />
              <v-row dense>
    ...
    <cc-solo-dialog
      ...
      title="Confirm End Round"
      @confirm="commitRoundEnd()"
    >
      <div class="flavor-text my-2">
        The following actors still have activations remaining:
      </div>
      <div
        v-for="a in initiative"
        :key="`diag_${a.ID}`"
        class="heading h3 text--text"
      >
        <v-icon left>{{ a.Icon }}</v-icon>
        {{ a.EncounterName }}
      </div>
      <div class="flavor-text my-2">
        Ending this round will cause these actors to lose their unspent

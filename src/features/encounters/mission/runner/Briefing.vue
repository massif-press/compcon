  ...
  <v-container style="margin-top: 50px; min-height: calc(100vh - 50px)">
    <v-row>
      <v-col>
        ...
        <p
          v-if="mission.Note"
          class="flavor-text panel text-center"
          v-html="mission.Note"
        />
        <div v-else class="flavor-text subtle--text text-center">
          // NO BRIEFING DATA //
        </div>
        <v-row justify="center">
          <v-col
            v-for="(s, i) in mission.Steps"
            :key="`step_${i}`"
            :cols="s.Name ? 3 : 1"
          >

  ...
  >
    ...
    >
      <template #item.type="{ item }">
        <v-chip
          :prepend-icon="isReady(item) ? 'mdi-check' : typeIcon(item.type)"
      ...
      <template #item.missing="{ item }">
        <v-chip
          v-for="name in item.missingLcpNames.slice(0, 3)"
          :key="name"
          size="x-small"
        ...
        <span
          v-if="item.missingLcpNames.length > 3"
          class="text-caption"
        >
        ...
        <v-menu width="400px">
          ...
          <cc-panel :title="$t('nav.v2Import.forceImportPanelTitle')">
            <v-card-text class="pa-1">
              ...
              <div class="mt-2">
                <v-chip
                  v-for="name in item.missingLcpNames"
                  :key="name"
                  size="x-small"
                  color="error"
                  class="mr-1 mb-1"
...
<script setup lang="ts">
  ...
  function isReady(item: any): boolean {
    if (item.type === 'pilot')
      return getV2PilotMissingLcps(item.originalData).missingIds.length === 0
    ...
    try {
      ...
      if (result.succeeded.length > 0) {
        notify({
          color: 'success',
          text: t(
            'nav.v2ImportNotify.reimportSuccessText',
      ...
      if (result.stillPending.length > 0) {
        notify({
          color: 'warning',
          text: t(
            'nav.v2ImportNotify.reimportPendingText',
            { count: result.stillPending.length },
            result.stillPending.length
          ),
        })
      }
      ...
      notify({
        color: 'error',
        text: t('nav.v2ImportNotify.reimportFailedText', { error: String(e) }),
      })
    }
    loading.value = false
  }
  ...
  async function reprocessSingle(backup: any) {
    ...
    try {
      ...
      notify({
        color: 'error',
        text: t('nav.v2ImportNotify.importFailedText', { error: String(e) }),
      })
    }
    loading.value = false
  }
  ...
  async function doForceImport(backup: any) {
    ...
    try {
      ...
      notify({
        color: 'error',
        text: t('nav.v2ImportNotify.forceImportFailedText', { error: String(e) }),
      })
    }
    loading.value = false
  }

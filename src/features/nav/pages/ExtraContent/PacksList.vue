      ...
      <template #item.Name="{ item }">
        ...
        >
          <template #activator="{ props }">
            ...
            >
              <v-chip
                v-for="p in patchesFor(item)"
                :key="p.id + 'badge'"
                size="x-small"
                :color="patchIsStale(p, item.Manifest.version) ? 'warning' : undefined"
              >
                {{ p.lang }}
              </v-chip>
            </v-btn>
          ...
          <v-card min-width="280">
            ...
            <v-list density="compact">
              ...
              >
                <template #prepend>
                  <v-icon
                    v-if="patchIsStale(p, item.Manifest.version)"
                    color="warning"
                    icon="mdi-alert"

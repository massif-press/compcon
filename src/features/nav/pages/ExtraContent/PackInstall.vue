  ...
  <v-card-text :style="mobile ? 'margin-top: 24px; padding: 8px' : 'margin-top: 8px'">
    ...
    >
      ...
      >
          ...
          >
            ...
            {{
              $t('nav.packInstall.byAuthorVersion', {
                ...
                version: alreadyInstalledVersion(pack),
              })
            }}
            <div
              class="ml-3 mb-2"
              ...
              >
                ...
                {{
                  $t('nav.packInstall.upgradeFromTo', {
                    from: alreadyInstalledVersion(pack),
                    to: pack.manifest.version,
                  })
                }}
              </v-chip>
              <v-chip
        ...
        <cc-alert
          v-if="contentPacks.some(x => !x.manifest.v3)"
          color="warning"
          class="my-3"
        >
          <span class="text-caption">
          ...
          <div
            v-for="pack in contentPacks.filter(x => !x.manifest.v3)"
            :key="pack.id"
            class="text-caption"
          >
            <b>{{ pack.manifest.name }}</b>
        ...
        <cc-alert
          v-if="hasUninstalledDependencies"
          color="error"
          class="my-3"
        >
          <span class="text-caption">
          ...
          >
            ...
            <div
              v-for="dep in uninstalledDependencies(pack)"
        ...
        <v-fade-transition mode="out-in">
          ...
          >
            {{
              $t(
                'nav.packInstall.installingPacks',
...
<script setup lang="ts">
  ...
  function gradeType(pack: IContentPack) {
    ...
    try {
      ...
      logger.error(
        `Removed ${pack.manifest.name || (pack.manifest as any).title || 'unknown LCP'} from import -- invalid version string breaks semver`,
        null,
        e
      )
      notify({

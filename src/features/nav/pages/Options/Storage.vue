    ...
    <v-card-text v-if="size.usage && size.quota"
      ...
      <div class="mb-4">
        ...
        <div class="text-caption text-right text-stark">
          {{ $t('nav.storage.warningDescription') }} {{ storageRange[0].toFixed(2) }}{{ $t('nav.storage.ofAvailableStorage') }}
          <b class="text-accent">{{ bytesToSize((storageRange[0] / 100) * size.quota) }}</b>
          {{ $t('nav.storage.hasBeenUsed') }}
        </div>
        <div class="text-caption text-right text-stark">
          {{ $t('nav.storage.maxDescription') }} {{ storageRange[1].toFixed(2) }}{{ $t('nav.storage.ofAvailableStorage') }}

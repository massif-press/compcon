...
<script setup lang="ts">
  ...
  const navSections = computed(() => {
    ...
    if (bond)
      sections.push({
        key: 'sel-bond',
        label: `${bond.Name} ${t('pm.sheet.powers')}`,
        accent: true,
        items: byBond(bond.ID),
      })
    sections.push({
      key: 'selected',
      label: t('pm.sheet.allSelectedPowers'),
      items: browserItems.value.filter(i => hasPower(i._p)),
    })
    allBonds.value
  ...
  function allowAdd(power: BondPower) {
    ...
    if (power.master) {
      ...
      return (
        !!bond && props.pilot.BondController.BondPowers.filter(x => x.origin === bond).length >= 4
      )
    }

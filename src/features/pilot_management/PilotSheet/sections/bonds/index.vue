        ...
        <v-row justify="center" align="center" style="height: 100%">
          <v-col cols="auto">
            <span class="heading h2 text--disabled"
              >Bonds available after License Level 1</span
            >
          </v-col>
        </v-row>
      </v-card>
        ...
        <v-col cols="auto" class="text-center">
          <v-row dense align="center">
              ...
              >
                ...
                <v-card width="400px">
                  <v-card-text class="text-center">
                    <i>
                      Set maximum pilot stress. Stress gains from sources like
                      selected Bond Powers are not automated
                    </i>
                    <v-text-field
            ...
            <v-col
              ...
              class="text-center"
              :style="
                pilot.BondController.AtMaxStress ? 'opacity: 1' : 'opacity: 0.7'
              "
            >
              <div>
                <v-icon
                  v-if="pilot.BondController.AtMaxStress"
                  size="60"
                  color="overcharge"
                >
                  mdi-heart-broken
                ...
                <v-menu v-model="addBondMenu" offset-y offset-x bottom left>
                  <template v-slot:activator="{ props }">
                    <v-btn
                      ...
                      class="mt-2"
                      :disabled="
                        !pilot.BondController.AtMaxStress ||
                        pilot.BondController.AtMaxBurdens
                      "
                      v-bind="props"
                    >
                      <v-icon start>mdi-plus</v-icon>
                      <span v-if="pilot.BondController.AtMaxBurdens"
                        >Burden Limit Reached</span
                      >
        ...
        <v-row justify="center">
          ...
          <v-col v-if="boon" cols="12">
            <v-card
              variant="outlined"
              :style="
                !pilot.BondController.HasVeteranPower ? 'opacity: 0.4' : ''
              "
            >
              <v-row
                no-gutters
...
export default {
  ...
  computed: {
    ...
    bid() {
      return this.pilot.BondController.Bond
        ? this.pilot.BondController.Bond.ID
        : 'none';
    },
    underlevel() {

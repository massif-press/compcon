      ...
      >
        ...
        >
          <template #item="{ element }">
            <div
              class="system-reorder-row mb-2"
              style="display: flex; align-items: flex-start; gap: 4px"
            >
              <div
                class="d-flex flex-column align-center system-drag-controls"
...
<script setup lang="ts">
  ...
  watch(
    () =>
      [...activeSystems.value, ...integratedSystems.value, ...moddedWeapons.value]
        .map(x => x.ID)
        .join('|') + `:${activeSystems.value.length}:${integratedSystems.value.length}`,
    updateSystemItems,
    { immediate: true }
  )

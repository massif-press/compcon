<template>
  <div
    v-if="item && !item.NoSynergies"
    :class="inline ? 'd-inline-block' : ''"
  >
    <div v-if="alert">
      <div
        v-for="(s, index) in synergies"
        :key="`synergy-${index}`"
      ...
      >
        ...
        >
          ...
          >
            <i18n-t
              keypath="pm.loadout.synergyWarning"
              tag="span"
              scope="global"
            >
              <template #not>
...
<script setup lang="ts">
  ...
  const synergies = computed(() => {
    return Synergy.Collect(props.location, props.mech as Mech, props.item)
  })
</script>

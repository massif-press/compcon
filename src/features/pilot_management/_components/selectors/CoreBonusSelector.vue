    ...
    <template #top>
      ...
      >
        ...
        >
          {{
            $t(
              'pm.selectors.coreBonusAvailable',
              { count: a.count, mf: a.manufacturer.ID },
              a.count
            )
          }}
        </v-chip>
...
<script setup lang="ts">
  ...
  const availability = computed<{ manufacturer: any; count: number }[]>(() => {
    ...
    return CompendiumStore()
      .Manufacturers.filter((m: any) => !m.IsHidden)
  ...
  function selectedCount(m: string): number {
    return props.pilot.CoreBonusController.CoreBonuses.filter((x: CoreBonus) => x.Source === m)
      .length
  }

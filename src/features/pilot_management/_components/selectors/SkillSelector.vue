...
<script setup lang="ts">
  ...
  const baseSkills = computed(() =>
    [
      ...filterByLcpConfig(CompendiumStore().Skills, props.pilot.LcpConfig),
      ...customSkills.value,
    ].sort((a, b) => {
  ...
  const activeSkillIds = computed(() =>
    props.pilot.SkillsController.Skills.map((s: any) => s.Skill.ID)
  )
  ...
  function addSkill(skill: any) {
    if (props.pilot.SkillsController.CanAddSkill(skill))
      props.pilot.SkillsController.AddSkill(skill)
  }
  function removeSkill(skill: any) {
    if (props.pilot.SkillsController.CanRemoveSkill(skill))

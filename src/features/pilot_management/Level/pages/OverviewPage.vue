      ...
      <div class="flavor-text my-4">
        <v-row justify="center">
          <v-col class="text-center text-stark">
            ...
            <div class="text-accent stat-text d-inline-block"
              style="position: relative; top: 10px; line-height: 10px">
              {{ pilot.Name }}
              <cc-slashes class="text-primary" />
              "{{ pilot.Callsign }}"
              <div class="text-cc-overline text-text pt-1"
      ...
      <v-row dense>
        ...
        <v-col cols="12"
          ...
          <ul>
            ...
            <li v-if="cbEligible"
              class="font-weight-bolder">{{ $t('pm.level.coreBONUSINSTALLATION') }}
            </li>
            <li v-else
              class="text-disabled">{{ $t('pm.level.ineligibleFORCOREBONUS') }}</li>
          </ul>
        </v-col>
      </v-row>
    </cc-panel>

  ...
  >
    ...
    <v-container class="flavor-text" style="font-size: 14px">
      ...
      <v-alert v-if="pilot.CBEligible" color="primary" outlined dense border="left" class="mt-2">
        ...
        <span
          class="overline"
        >Implementing CORE augmentation protocols from a nonrecognized, phohibited, or unregistered Licensor or Licensors renders the operating pilot liable for any and all damages resultant from CORE malfunction, up to and including reactor meltdown.</span>
      </v-alert>
    </v-container>
    <cc-core-bonus-selector v-if="pilot.CBEligible" level-up :pilot="pilot" />
    <v-card v-else flat tile color="grey lighten-2">
      <v-card-text class="flavor-text text-center py-5 px-3">
        <span class="heading h2 grey--text text--darken-1">PILOT INELIGIBLE FOR CORE BONUS</span>
        <br />
        <span
          class="grey--text text--darken-2"
        >NEXT CORE BONUS IN {{ 3 - (pilot.Level % 3) }} LICENSE LEVELS</span>
      </v-card-text>
    </v-card>
  </cc-stepper-content>
</template>

<template>
  ...
  >
    ...
    <v-col class="pt-1 ml-n1">
      <v-row
        dense
        :class="`px-2 ${isSelected ? 'selected-gradient' : 'gradient'}`"
      >
        <v-col class="text-white">
          <div
            v-show="!$vuetify.display.mdAndDown"
            class="text-overline mt-n1"
          >
            {{ template.code }}
          </div>
        ...
        >
          ...
          >
            <span v-if="!isSelected">
              {{ $t('pm.new.selectTemplate', { name: template.name }) }}
            </span>
      ...
      <div
        v-html-safe="template.description"
        class="pa-2"
      />
      <div
      ...
      >
        ...
        <div class="panel clipped py-1 px-2 my-2">
          ...
          >
            <v-col
              v-for="(m, index) in template.build.mech.mounts"
              :key="`mount-${index}`"
            >
              <div class="flavor-text text-stark text-center">{{ mountLabel(m.mount_type) }}</div>
              <v-row dense>
                <v-col
                  v-for="(w, index) in m.slots"
                  :key="`slot-${index}`"
                  class="text-center"
                >
                  <cc-item-modal
                    class="mx-1"
                    :item="item('MechWeapons', w)"
                  />
                </v-col>
              </v-row>
            </v-col>

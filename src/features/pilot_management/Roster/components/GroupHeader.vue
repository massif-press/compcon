<template>
  ...
  >
    <v-icon
      v-if="!noGroup && (!mobile || dragModeActive)"
      icon="mdi-drag"
    ...
    />
    <v-avatar
      v-if="group.PortraitController.HasImage"
      size="30px"
      class="mr-2"
    >
  ...
  <v-expand-transition>
    ...
    >
      <v-row align="start">
        ...
        <v-col
          v-if="group.PortraitController.CloudImage || edit"
          cols="12"
          md="3"
          :order="mobile ? 0 : 1"
          :class="!mobile ? 'mb-2' : ''"
              ...
              >
                ...
                >
                  <v-list-item
                    ...
                    :subtitle="pilot.Name"
                    slim
                    @click="emit('transfer-pilot', <Pilot>pilot)"
                  />
                </v-list>
              </v-card>
            </v-menu>
  ...
  const transferrable = computed(() =>
    PilotStore().Pilots.filter(
      pilot =>
        ...
        (!search.value ||
          (pilot.Name + pilot.Callsign).toLowerCase().includes(search.value.toLowerCase()))
    )
  )

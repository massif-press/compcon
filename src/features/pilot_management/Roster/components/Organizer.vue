<template>
  ...
  >
    ...
    <template #item.Mech="{ item }">
      {{
        (item as any).FavoriteMech
          ? (item as any).FavoriteMech.Name
          : (item as any).ActiveMech
            ? (item as any).ActiveMech.Name
            : 'None'
      }}
    </template>
    ...
    <template #dialogs>
      <v-dialog
        v-model="setGroupDialog"
        max-width="500px"
      >
        <v-card>
          <v-toolbar density="compact">
            <v-toolbar-title>{{ $t('pm.roster.setGroup') }}</v-toolbar-title>
            <v-spacer />
            <v-btn
              icon
...
<script setup lang="ts">
  ...
  function openSetGroup(selected: string[]) {
    sel.value = selected

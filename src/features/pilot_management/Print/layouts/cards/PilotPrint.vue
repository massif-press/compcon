  ...
  <div class="text-black pa-3">
    <v-row justify="space-around">
      <card>
        ...
        <div
          :style="pilot.HasPortrait ? `background-image: url('${pilot.Portrait}')` : ''"
          style="
            height: 100px;
            width: 100%;
        ...
        >
          ...
          <v-col>
            <div class="font-weight-bold caption text-primary text-primary">
              {{ $t('stats.agi') }}
            </div>
      ...
      <card>
        <div>
          <div
            class="caption text-center mt-n1 mb-2"
      ...
      <card v-if="pilot.CoreBonusController.CoreBonuses.length">
        ...
        <v-divider />
        <div
          v-for="cb in pilot.CoreBonusController.CoreBonuses"
          :key="cb.ID"
          class="my-2"
        >
          <div
      ...
      <action-card
        v-for="a in gearActions"
        :key="a.ID"
        :action="a"
        :header="$t('common.pilotGear')"
      />
      <action-card
        v-for="a in talentActions"
        :key="a.ID"
        :action="a"
        :header="$t('common.pilotTalent')"
      />
      <deployable-card
        v-for="(d, i) in gearDeployables"
        :key="`gear-deployable-${i}`"
        :deployable="d.ItemData"
        :header="$t('common.pilotGear')"
      />
      <deployable-card
        v-for="(d, i) in talentDeployables"

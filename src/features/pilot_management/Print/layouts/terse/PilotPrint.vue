<template>
  <div class="text-black pa-2">
    <v-row dense
      ...
      <v-col v-if="blank && landscape"
        cols="auto">
        <div class="text-caption text-primary mb-n1">
          <span class="text-uppercase">{{ $t('common.name') }}</span>
        </div>
    ...
    <v-row dense
      ...
      <v-col>
        ...
        <v-row v-if="blank"
          ...
          <v-col v-for="n in 12"
            :key="`talent-${n}`"
            :cols="landscape ? (hasPilotOption('pilotPortrait') ? 6 : 3) : 6">
            <blank-line :height="24"
              inline />
          </v-col>
        </v-row>
        <v-chip v-for="t in pilot.TalentsController.Talents"
          v-else-if="hasPilotOption('separateTalentDetail')"
          :key="t.Talent.ID"
          label
          variant="outlined"
          size="small"
      ...
      <v-col v-if="hasPilotOption('pilotPortrait')"
        cols="4"
        class="mt-5">
        <v-card height="100%"
          variant="outlined"
    ...
    <div v-if="hasLoadout || blank"
      class="text-caption mb-n3 text-primary">
      {{ $t('common.pilotLoadout') }}
    </div>
    <v-row v-if="armorItems.length || weaponItems.length || blank"
      dense
      justify="space-between"
      class="mt-n1 caption">
      <v-col v-for="a in armorItems"
        :key="a.ID"
    ...
    <v-row v-if="gearItems.length || blank"
      dense
      justify="space-between"
      class="mt-n1 caption pb-3">
      <v-col v-for="g in gearItems"
        :key="g.ID"
        style="position: relative">
    ...
    <v-row v-if="hasPilotOption('extraEquipmentSpace')"
      dense
      justify="space-between"
    ...
    <div v-if="blank"
      ...
      <div class="text-caption mb-n2 mt-1 text-primary">{{ $t('common.reserves') }}</div>
      <v-row dense>
        <v-col v-for="r in hasPilotOption('extraReserveSpace') ? 9 : 6"
          :key="`reserve-${r}`"
          cols="4">
          <fieldset class="mt-2"
            style="position: relative">
    ...
    <div v-if="hasPilotOption('appearanceNotes')">
    ...
    <div v-if="hasPilotOption('pilotBiography')">
      <div class="text-overline text-primary"
        style="line-height: 0">
        {{ $t('pm.new.biography') }}
      </div>
    ...
    <div v-if="hasPilotOption('pilotNotes')">
      <div class="text-overline text-primary"
        style="line-height: 0">
        {{ $t('common.notes') }}
      </div>
  ...
  <fieldset v-if="hasPilotOption('appendLined')"
  ...
  <fieldset v-if="hasPilotOption('appendUnlined')"
    class="mx-1 my-3 px-3">
    <div class="mb-4">
      <notes :rows="16" />
    </div>
  ...
  <div v-for="t in pilot.TalentsController.Talents"
    v-if="hasPilotOption('separateTalentDetail')"
    :key="t.Talent.ID"
    no-gutters
    justify="space-between"

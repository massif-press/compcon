<template>
  <div class="text-black px-2 pb-2">
    ...
    >
      <v-col cols="auto">
        <div class="text-overline mt-n2 mb-n2 text-primary">
          <span class="text-uppercase">{{ $t('pm.link.bond') }}</span>
        </div>
        <div
          v-if="blank"
    ...
    >
      <v-col
        v-for="(q, i) in bc.Bond.Questions"
        v-show="bc.Answers[i]"
        :key="`question-${i}`"
      >
        <div
          class="text-overline mt-n5"
          ...
          <fieldset>
            ...
            >
              <v-col
                ...
                class="mr-4"
              >
                <span class="heading ph2 text-grey">&emsp;/{{ b.Segments }}</span>
              </v-col>
              <v-col>
...
<script setup lang="ts">
  import { CONTENT, ORIENTATION } from '@/ui/print/options'
  ...
  const blank = computed(() => {
    return props.options.content.key === CONTENT.blank.key
  })

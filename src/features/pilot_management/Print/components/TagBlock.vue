<template>
  <div v-if="show()"
    class="px-4">
    <v-card v-for="(t, index) in (tags as any[])"
      :key="`tag-${index}`"
      class="pa-1 my-n1"
      variant="plain">
      <v-row no-gutters>
        <v-col cols="auto"> <v-icon start
            size="x-small"
            icon="mdi-tag-outline" /> </v-col><v-col>
          <div class="text-caption"
            style="line-height: 14px">
            <b>{{ t.GetName() }}</b>: {{ t.GetDescription() }}
          </div>
        </v-col>
      </v-row>
    </v-card>
  ...
  <div v-else
    ...
    <v-chip v-for="(t, index) in (tags as any[])"
      v-show="showTag(t.ID)"
      :key="`chip-${index}`"
      size="x-small"
      label

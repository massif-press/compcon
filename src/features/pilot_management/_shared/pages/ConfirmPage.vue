<template>
  ...
  >
    ...
    <cc-alert
      v-else-if="context === 'level' && !pilotReady"
      color="error"
    >
      ...
      <span v-if="context === 'new'">
        {{
          $t('pm.shared.registerNewPilot', {
            callsign: pilot.Callsign || default_callsign,
            name: pilot.Name || default_name,
          })
        }}
      </span>
      <span v-else>
...
<script setup lang="ts">
  ...
  const pilotComplete = computed(() => {
    ...
    if (props.context === 'new')
      return pilotReady.value && p.LicenseController.HasLicenses && p.CoreBonusController.HasCBs
    return !!(
      p.Callsign &&
      p.Name &&
      p.SkillsController.HasFullSkills &&
  ...
  const missingBasicInfo = computed(
    () => !(props.pilot as any).Callsign || !(props.pilot as any).Name
  )

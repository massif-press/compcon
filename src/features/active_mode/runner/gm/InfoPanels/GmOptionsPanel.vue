  ...
  <v-card>
        ...
        >
          ...
          <v-expansion-panel-text class="bg-background pa-0 mx-n5">
            ...
            <div v-if="combatant.actor.StatController">
              <v-row
                v-for="stat in combatant.actor.StatController.DisplayKeys"
                :key="stat.key"
                dense
              ...
              >
                ...
                <v-col>
                  <div class="text-cc-overline ml-4 text-disabled">
                    {{ $t('active.gmOptions.max') }}
                  </div>
                  <cc-number-field
                    v-model="combatant.actor.StatController.MaxStats[stat.key]"
                    color="exotic"
                  />
                </v-col>
              </v-row>
    ...
    <v-card-text>
      <div class="text-cc-overline text-disabled mb-1">
        {{ $t('active.gmOptions.editReinforcements') }}
      </div>
      <i
        v-if="!reinforcements.length"
        class="text-text ml-2"
      >
        {{ $t('active.gmOptions.noReinforcementSchedule') }}
      </i>
      <v-row
        v-for="combatant in reinforcements"
        :key="combatant.id"
        dense
        align="center"
      >
...
<script setup lang="ts">
  ...
  watch(expandedOverridePanel, val => {
    ...
    snapshot(
      ...
      t('active.gmOptions.undoEditOverrides', { name: combatant.actor.Name })
    )
  })

  function removeActor(actor: any) {
    const idx = (props.encounterInstance as any).Combatants.findIndex(
  ...
  function manualSave() {
    try {
      ...
      notify({
        ...
        text: t('active.encounter.saveSuccessfulText', {
          name: (props.encounterInstance as any).Encounter.Name,
          round: (props.encounterInstance as any).Round,
        }),
      ...
      notify({
        ...
        text: t('active.encounter.saveFailedText', {
          name: (props.encounterInstance as any).Encounter.Name,
        }),

<template>
  <v-card>
    ...
    <v-card-subtitle class="text-cc-overline">
      <span
        v-if="encounter.Environment.Name !== 'Default'"
    ...
    >
      ...
      <div
        v-for="(item, index) in encounter.NarrativeController.TextItems"
        :key="`text-${index}`"
      >
    ...
    <v-card-text class="py-2">
      ...
      <div
        v-for="block in sitrepBlocks"
        :key="block"
      >
        <div
          v-if="encounter.Sitrep[block].length"
          class="mb-4"
        >
          <div class="text-cc-overline">{{ block }}</div>
          <div
            v-if="block === 'Conditions'"
            class="pr-12"
          >
            <cc-panel
              v-for="c in encounter.Sitrep.Conditions"
              :key="c.title"
              :title="c.title"
            >
              {{ c.condition }}
    ...
    <v-card-text class="py-2">
      ...
      <p
        v-html-safe="encounter.Environment.Description"
        class="text-text"
      />
      <div class="text-right mt-n2">
        <v-btn

  ...
  <div>
    ...
    >
      ...
      <span class="text-cc-overline">
        {{ $t('active.panelBase.itemDestroyed', { type: combatant.ItemType }) }}
      </span>
    </cc-alert>
    <cc-alert
      v-else-if="!combatant.IsDeployed"
      class="ma-2"
      prominent
      icon="cc:drone"
    ...
    >
      <i18n-t
        keypath="active.deployPanel.recalledAwaiting"
        tag="span"
        scope="global"
      >
        <template #type>{{ combatant.ItemType }}</template>
        <template #redeployment>
          <b>{{ $t('active.deployPanel.redeployment') }}</b>
        </template>
      </i18n-t>
      <cc-button
        size="small"
      ...
      >
        <div class="px-4">
          <v-icon
            :icon="getActionIcon(redeploy)"
            class="mt-n1"
            start
          />
          {{ $t('active.deployPanel.redeploy') }}
          <span class="text-disabled">({{ redeploy }})</span>
        </div>
        <template #options>
          <v-list
            density="compact"
    ...
    <v-scroll-y-reverse-transition>
      <div v-if="!hide">
        ...
        >
          <div
            v-if="combatant.Base.Detail"
            v-html-safe="combatant.Base.Detail"
          />
          <cc-combat-action-chip
            v-for="action in combatant.Base.Actions"
            :key="action.ID"
        ...
        >
          <v-icon
            :icon="getActionIcon(recall)"
            class="mt-n1"
            start
          />
          {{ $t('active.deployPanel.recall') }}
          <span class="text-disabled">({{ recall }})</span>
          <template #options>
            <v-list
              density="compact"
              class="pa-0"

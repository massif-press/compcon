<template>
  <div
    v-if="item.CombatController.TimedEffects.length"
    class="text-cc-overline text-disabled mt-2"
  >
    {{ $t('ui.cascade.pending') }}
  </div>
  <v-row
    v-for="(t, idx) in item.CombatController.TimedEffects"
    :key="`timed-${idx}`"
    dense
    align="center"
  ...
  >
    ...
    <v-col>
      <div class="text-cc-overline">
        {{ t.Name }}
        <span v-if="t.Origin">// {{ t.Origin }}</span>
      </div>
...
<script setup lang="ts">
  ...
  function apply(effect: TimedEffect, index: number) {
    ...
    if (e) {
      if (e.damage)
        e.damage.forEach((d: any) => props.item.CombatController.TakeDamage(d.type, d.value))

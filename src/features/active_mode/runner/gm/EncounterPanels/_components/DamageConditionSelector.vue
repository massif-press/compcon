  ...
  <div>
    ...
    >
      ...
      >
        ...
        >
          <div
            v-if="layout.showIcon || (layout.showIcon && hasImmunity(damage))"
          ...
          <div
            ...
            class="text-cc-overline damage-label"
          >
            {{ $t(damage.label) }}
          </div>
          <div
            v-if="layout.showLabel && conditionLabel(damage)"
            class="text-caption damage-condition"
          >
    ...
    <v-scroll-y-reverse-transition>
      <div
        v-if="specialResists.length"
        class="my-1"
      >
        <v-card
          v-for="r in specialResists"
          :key="r.type"
          flat
          tile
          border
...
<script setup lang="ts">
  ...
  const vulnerabilities = computed(() => {
    return props.controller.Resistances.filter(x => x.condition === 'vulnerable').map(x => x.type)
  })
  const immunities = computed(() => {
    return props.controller.Resistances.filter(x => x.condition === 'immunity').map(x => x.type)
  })
  const resistances = computed(() => {
    return props.controller.Resistances.filter(x => x.condition === 'resistance').map(x => x.type)
  })

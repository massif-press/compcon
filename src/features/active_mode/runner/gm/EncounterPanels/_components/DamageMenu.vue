        ...
        <v-card-text>
          <v-row>
            ...
            >
              <div class="text-cc-overline text-disabled">
                {{ $t('active.damageMenu.incomingValue') }}
              </div>
              <v-divider />
              <v-text-field
                ref="damageInput"
            ...
            >
              <div class="text-cc-overline text-disabled">
                {{ $t('active.damageMenu.defenderStatus') }}
              </div>
              <v-divider class="mb-2" />
              <v-row
                v-if="controller.StatController.CurrentStats['armor']"
                no-gutters
                justify="center"
              ...
              >
                ...
                <v-col
                  class="text-cc-overline mt-1"
                  :style="damageMods.includes('ap') ? 'text-decoration: line-through' : ''"
                >
                  {{
                    $t('active.damageMenu.armorN', {
                      n: controller.StatController.CurrentStats['armor'] || 0,
                    })
                  }}
                </v-col>
              </v-row>
              <v-row
                v-for="damage in controller.Resistances"
              ...
              <v-card
                v-if="!getActiveStatuses.length && !controller.Resistances.length"
                flat
                tile
            ...
            <v-col>
              <div class="text-cc-overline text-disabled">
                {{ $t('active.damageMenu.totalDamage') }}
              </div>
              <v-divider class="mb-2" />
              <v-card
                flat
          ...
          >
            <v-col cols="auto">
              ...
              >
                {{ $t('active.damageMenu.applyClose') }}
              </cc-button>
            </v-col>
          </v-row>
        </v-card-text>
...
<script setup lang="ts">
  ...
  const getActiveStatuses = computed(() => {
    ...
    return props.controller.Statuses.filter(x =>
      relevantStatuses.some(r => r.id === x.status.ID)
    ).map(s => relevantStatuses.find(as => as.id === s.status.ID))
  })

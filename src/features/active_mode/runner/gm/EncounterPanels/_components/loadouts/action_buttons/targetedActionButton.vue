      ...
      <div class="pa-2">
        ...
        <v-row dense
          ...
          <base-target-selector v-else
            :event="event" />
          <base-attack-roller v-if="melee && !pc"
            :event="event" />
        </v-row>

        <v-btn-toggle v-if="contested || (pc && melee)"
          v-model="succeeded"
          mandatory
          density="compact"
          class="my-2">
...
function apply(close: () => void, force = false) {
  ...
  if (pc.value) {
    controller.value.PerformAction(props.action.ID, { success: succeeded.value, force })
  } else {
    targets.forEach((t, i) => {
      const opts = {

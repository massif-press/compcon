<template>
  <div>
    <div>
      ...
      <cc-alert
        v-if="weapon.IsLimited && weapon.Uses >= weapon.MaxUses && !weapon.CanSetUses"
        color="warning"
        class="my-2"
      >
        <v-icon
    ...
    <div v-if="weapon.Profiles.length > 1">
      <v-btn
        v-for="(profile, index) in weapon.Profiles"
        :key="`profile_${index}`"
        size="small"
        flat
        tile
    ...
    <cc-panel
      v-for="(e, index) in event.TargetEvents"
      :key="`target-${index}`"
      :title="e.Effect.Name"
      density="compact"
      variant="tonal"
...
<script setup lang="ts">
  ...
  const canUse = computed(() => {
    ...
    return (
      !props.profile.Used &&
      !props.profile.Parent.Destroyed &&

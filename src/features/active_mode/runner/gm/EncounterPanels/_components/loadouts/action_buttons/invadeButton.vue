  ...
  <cc-dialog
    ...
    <template #default="{ close }">
      <v-sheet class="d-flex ml-n2">
        <v-tabs v-model="tab" direction="vertical" density="compact">
          <v-tab height="30" value="invade">Invade</v-tab>
          <v-divider />
          <div class="pa-2 text-cc-overline text-disabled">Available Invade Actions</div>
          <v-tab height="30" v-for="item in invadeActions" :key="item.ID" :value="item.ID">
            <v-icon :icon="item.Icon" class="mr-2" />
            {{ item.Name }}
          </v-tab>
        </v-tabs>
        <v-divider vertical class="mr-1" />
        <v-tabs-window v-model="tab" class="px-2">
          <div v-if="tab === 'invade'">
            <div class="heading h4">{{ action.Name }}</div>
            <p class="text-text pl-2" v-html="action.Detail" />
            <v-row dense align="center" class="my-2">
              <v-col><v-divider /></v-col>
              <v-col class="heading text-disabled" cols="auto">Select an Invade Action</v-col>
              <v-col><v-divider /></v-col>
            </v-row>
          </div>
          <menu-input

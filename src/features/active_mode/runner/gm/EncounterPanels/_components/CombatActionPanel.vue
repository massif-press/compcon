    ...
    <v-row dense
      ...
      <template v-for="(action, idx) in actions"
        ...
        <v-col v-else
          ...
          md="4"
          lg="">
          <v-btn :color="action.available ? action.color : 'grey'"
            flat
            tile
            block
            :class="[
            ...
            <v-icon v-if="!action.available && !action.readonly && !layout.showLabel"
              class="combat-action-spent-mark"
              size="14"
              icon="mdi-close-thick" />
            <v-tooltip v-if="!layout.showLabel"
              activator="parent"
              location="top"
              :text="action.label" />
          </v-btn>
        </v-col>
        <v-responsive v-if="action.breakAfter && layout.showLabel"
          :key="`b${idx}`"
          width="100%" />
      </template>
    </v-row>
...
const actions = computed<Tile[]>(() => {
  ...
  const tiles: Tile[] = [
    ...
    { key: 'sep', separator: true },
    {
      ...
      available: c.CanActivate('quick1'),
      toggle: () => c.toggleCombatAction('quick1'),
    },
    {
      key: 'quick2',
      icon: inOvercharge.value ? 'cc:overcharge' : 'mdi-hexagon-slice-3',
      ...
      color: inOvercharge.value ? 'action--overcharge' : 'action--quick',
      available: c.CanActivate('quick2'),
      toggle: () => c.toggleCombatAction('quick2'),
      breakAfter: true,
    },
    {
      key: 'move',

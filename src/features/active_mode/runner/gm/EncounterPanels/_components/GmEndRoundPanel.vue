    ...
    <template #default="{ isActive }">
      <v-card-text>
        ...
        <div v-if="nextRoundAlerts">
          ...
          >
            <div
              v-for="(s, index) in getTimeoutStatuses(a)"
              :key="`timeout-${index}`"
            ...
            >
              ...
              <div
                v-for="(s, sIdx) in getTimeoutStatuses(a)"
              ...
              >
                ...
                <b class="text-accent">
                  {{
                    $t('active.gmEndRound.endsOnTargetTurn', {
                      when: s.expires.EndsOn,
                      target: getStatusTarget(s.expires.ExpirationActorID, a.CombatName),
                    })
                  }}
                </b>
              </div>
            ...
            <div
              v-for="(s, index) in getTimeoutStatuses(a, true)"
          ...
          >
            ...
            <i18n-t
              keypath="active.gmEndRound.braceExit"
              tag="span"
              scope="global"
            >
              <template #braced>
...
<script setup lang="ts">
  ...
  const nextRoundAlerts = computed(() => {
    return (
      braced.value.length ||
      activeActors.value.some(
        c => getTimeoutStatuses(c).length || getTimeoutStatuses(c, true).length
      )
    )
  })
  ...
  const unendedTurns = computed(() => {
    return activeActors.value.filter(
      c =>
        c.StatController.MaxStats['activations'] && c.StatController.CurrentStats['activations'] > 0
    )
  })
  const hasTimedEffects = computed(() => {
    return activeActors.value.filter(
      c => c.EndOfTurnEffects().length > 0 || c.TimedEffects.length > 0
    )
  })

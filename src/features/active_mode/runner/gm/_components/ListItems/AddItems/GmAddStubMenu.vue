...
<script setup lang="ts">
  ...
  const canAdd = computed(() => {
    if (newPlaceholder.value.type === 'pilot') {
      return (
        !newPlaceholder.value.name.trim().length || !newPlaceholder.value.Mechname.trim().length
      )
    }
    return !newPlaceholder.value.name.trim().length
  })

  function addAndClose(isActive: { value: boolean }) {
    add()

          ...
          <v-col cols="auto" style="position: relative">
            <v-icon
              ...
              style="
                ...
                border-radius: 4px;
              " />
            <v-img height="100%" width="60px" cover :src="item.frame.DefaultImage" />
          </v-col>
          <v-col v-if="!collapsed" class="mx-1">
            <div>
              <span class="heading h4">{{ item.callsign }}</span>
              <span class="text-caption text-disabled">
                <cc-slashes />
                {{ item.name }}
              </span>
            </div>
            <div class="text-cc-overline">{{ item.frame.Source }} {{ item.frame.Name }}</div>
            <v-row dense justify="space-between" style="font-size: 16px">
              <v-col cols="auto" v-for="(stat, index) in item.stats" :key="index">
                <span v-if="stat.max > 0 && index !== 'repair'">
            ...
            <div v-for="status in item.statuses" class="mb-1">

    ...
    >
      ...
      <v-card-text class="text-text pa-1">
        <div v-if="!isMigrating && !migrationResult">
          ...
          <div v-else>
            <i18n-t
              keypath="mainMenu.migration.migrationDesc"
              tag="span"
              scope="global"
            >
              <template #detected>
                <b class="text-accent">{{ $t('mainMenu.migration.detectedText') }}</b>
              </template>
            </i18n-t>
          </div>
          <div v-if="detectionData">
            <v-list-item
              v-if="detectionData.pilots"
              class="bg-background my-1"
              prepend-icon="cc:pilot"
            >
              {{
                $t(
                  'mainMenu.migration.pilotsDetected',
                  { count: detectionData.pilots },
                  detectionData.pilots
                )
              }}
            </v-list-item>
            <v-list-item
              v-if="detectionData.npcs"
              class="bg-background my-1"
              prepend-icon="cc:frame"
            >
              {{
                $t(
                  'mainMenu.migration.npcsDetected',
                  { count: detectionData.npcs },
                  detectionData.npcs
                )
              }}
            </v-list-item>
            <v-list-item
              v-if="detectionData.encounters"
              class="bg-background my-1"
              prepend-icon="cc:encounter"
            >
              {{
                $t(
                  'mainMenu.migration.encountersDetected',
                  { count: detectionData.encounters },
                  detectionData.encounters
                )
              }}
            </v-list-item>
            <v-list-item
          ...
          <div class="my-1 text-caption text-center">
            <i18n-t
              keypath="mainMenu.migration.v2NotModified"
              tag="span"
              scope="global"
            >
              <template #not>
                <strong>
                  <span class="text-lowercase">{{ $t('pm.loadout.not') }}</span>
                </strong>
              </template>
            </i18n-t>
          </div>
        ...
        <div v-if="migrationResult && !isMigrating">
          ...
          <div v-else>
            <v-list-item
              v-if="migrationResult.pilotsImported"
              class="bg-background my-1"
            >
              <v-icon
                start
            ...
            <v-list-item
              v-if="migrationResult.pilotsBackedUp"
              class="bg-background my-1"
            >
              <v-icon
                start
            ...
            <v-list-item
              v-if="migrationResult.npcsImported"
              class="bg-background my-1"
            >
              <v-icon
                start
            ...
            <v-list-item
              v-if="migrationResult.npcsBackedUp"
              class="bg-background my-1"
            >
              <v-icon
                start
            ...
            <v-list-item
              v-if="migrationResult.encountersImported"
              class="bg-background my-1"
            >
              <v-icon
                start
            ...
            <v-list-item
              v-if="migrationResult.encountersBackedUp"
              class="bg-background my-1"
            >
              <v-icon
                start
            ...
            <v-list-item
              v-if="migrationResult.lcpsImported"
              class="bg-background my-1"
            >
              <v-icon
                start
            ...
            <v-expansion-panels
              v-if="migrationResult.errors && migrationResult.errors.length > 0"
              class="mt-2"
            >
              <v-expansion-panel>
                ...
                <v-expansion-panel-text>
                  <div
                    v-for="err in migrationResult.errors"
                    :key="err"
      ...
      <v-card-actions>
        ...
        <v-spacer />
        <cc-button
          v-if="!migrationResult && !isMigrating"
          color="success"

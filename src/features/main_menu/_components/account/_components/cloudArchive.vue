    ...
    <div v-else>
      ...
      >
        ...
        <template #item.size="{ item }">
          ...
          <span v-else>
            {{ ((item.size ?? 0) / 1024).toFixed(2) }} {{ $t('mainMenu.unit.kb') }}
          </span>
        </template>
          ...
          >
            ...
            <v-card-text>
              ...
              <div
                ...
                v-text="
                  `Delete old archives if cloud archives take up more than ${settings.autoBackupPrunePct}% of your total cloud storage limit`
                "
              />
            </v-card-text>
            <v-divider />
            <v-card-actions>
              ...
              <v-btn
                ...
                @click="prune"
              >
                <span v-if="prunableItemCount">
                  {{
                    $t(
                      'mainMenu.archive.pruneItems',
                      { count: prunableItemCount },
                      prunableItemCount
                    )
                  }}
                </span>
  ...
  async function createNew() {
    ...
    if (cooldown + cooldownTime > Date.now()) {
      notify({
        ...
        text: t('notify.account.cloudArchiveCooldownText', {
          seconds: Math.ceil((cooldown + cooldownTime - Date.now()) / 1000),
        }),
        icon: 'mdi-clock',
        type: 'error',
      })

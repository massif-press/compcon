  ...
  <div>
    <div class="d-flex align-center pa-2 gap-2">
      ...
      >
        {{
          bulkDeleteMode
            ? $t('mainMenu.dataItem.cancelBulkSelection')
            : $t('mainMenu.dataItem.bulkDelete')
        }}
      </cc-button>
      <cc-button
        v-if="bulkDeleteMode && selectedForDelete.length"
        size="small"
        color="error"
    ...
    >
      ...
      <template #item.ItemType="{ item }">
        ...
        <span v-else-if="item.ItemType === 'EncounterInstance'">
          {{ $t('mainMenu.dataItem.typeActiveEncounter') }}
        </span>
        <span v-else-if="item.ItemType === 'EncounterArchive'">
          {{ $t('mainMenu.dataItem.typeArchivedEncounter') }}
        </span>
      ...
      <template #item.localLastModified="{ item }">
        <span v-if="item.SaveController?.LastModified || item.SaveController?.Created">
          {{
            new Date(
              item.SaveController.LastModified || item.SaveController.Created
            ).toLocaleString()
          }}
        </span>
        <i
      ...
      <template #item.code="{ item }">
        ...
        <span v-else-if="item._isRemote && item.SaveController?.RemoteCode?.length > 0">
          {{
            `${item.SaveController.RemoteCode.slice(0, 4)}-${item.SaveController.RemoteCode.slice(
              4,
              8
        ...
        <span v-else-if="item.CloudController?.Metadata?.Code?.length > 0">
          {{
            `${item.CloudController.Metadata.Code.slice(
              0,
              4
            )}-${item.CloudController.Metadata.Code.slice(
              4,
      ...
      <cc-alert color="error">
        <v-toolbar-title>
          <span class="heading h3">
            {{
              $t(
                'mainMenu.dataItem.deleteNItems',
                {
...
<script setup lang="ts">
  ...
  const { sync, forceSyncLocal, forceSyncCloud } = useItemTransfer({
  ...
  function isItemSynced(item: any): boolean {
    if (item._isRemote) {
      return (
        (item.CloudController?.Metadata?.Updated ?? 0) >= (item.SaveController?.LastModified ?? 0)
      )
    }
  ...
  const shownItems = computed(() => {
    ...
    const baseItems = allSyncableItems.value
      .filter((item: any) => {
        const t = normalizeItemType(item.ItemType)
      ...
      .map((item: any) => ({
        ...item,
        _isRemote: false,
        _isBrokenRemote: false,
        _isChild: false,
        _parentId: null,
      }))

    const remoteItems = allRemoteItems.value
      .filter((item: any) => {
        const t = normalizeItemType(item.ItemType)
        if (typeFilter.length && !typeFilter.includes(t)) return false
        if (props.search && !item.Name.toLowerCase().includes(props.search.toLowerCase()))
  ...
  function itemLine1(item: any): string {
    ...
    if (t === 'pilot') return item._callsign || item._name || item.Name || 'Unknown Pilot'
      ...
      return item._name || item.Name || Unit.GetDefaultName(item) || 'Unknown Unit'

    ...
    ICombatant
{
  ...
  public LoadError(err: any, message: string): void {
    logger.error(`Pilot ${this.ID} (${this.Callsign}) failed to load ${message}; ${err}`, this, err)
  ...
  private static serializeSE(equipment: CompendiumItem[]): IUnlockData {
    return {
      ...
      MechWeapons: equipment.filter(x => x.ItemType === ItemType.MechWeapon).map(i => i.ItemData),
      WeaponMods: equipment.filter(x => x.ItemType === ItemType.WeaponMod).map(i => i.ItemData),
      MechSystems: equipment.filter(x => x.ItemType === ItemType.MechSystem).map(i => i.ItemData),
    }
  }

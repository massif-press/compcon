...
class EidolonLayerSaveData implements IFeatureContainer, ICombatant {
  ...
  public SetActivations(playerCount: number) {
    const bonusActivations = Math.min(4, Math.floor(playerCount / 2))
    this.StatController.setMax('activations', Math.max(1, bonusActivations))
  }

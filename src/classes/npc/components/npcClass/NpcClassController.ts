...
class NpcClassController {
  ...
  public set Tier(newTier: number) {
    this._tier = newTier;
    this.Parent.StatController = new StatController(
      this.Parent,
      this.ClassStats(newTier)
    );
  ...
  public static Deserialize(parent: Npc, data: INpcClassSaveData) {
    ...
    parent.NpcClassController._class = CompendiumStore().referenceByID(
      'NpcClasses',
      data.class
    );

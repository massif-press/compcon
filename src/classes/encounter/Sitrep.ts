...
class SitrepData {
  ...
  constructor(id?: string, hydrate?: boolean) {
    ...
    let s;
    if (id) {
      s = CompendiumStore().referenceByID('Sitreps', id) as Sitrep;
    } else {
      s = CompendiumStore().getItemCollection('Sitreps')[0] as Sitrep;
    }
    this.Name = s.name;
    ...
    if (s.noVictory)
      this.Conditions.push({ title: 'Stalemate', condition: s.noVictory });

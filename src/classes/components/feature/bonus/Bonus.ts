...
class Bonus {
  ...
  private parseDetail(detail): string {
    ...
    str = str.replace(
      /{INC_DEC}/g,
      !isNaN(this.Value as number) && (this.Value as number) > -1
        ? 'Increases'
        : 'Decreases'
    );
    str = str.replace(
  ...
  public static SumStatic(item: IBonusDataContainer, bonusId: string): string {
    const bArr =
      item.bonuses?.filter((x) => x.id === bonusId).map((y) => y.val) || [];
  ...
  public static Int(
    base: number,
    id: string,
    source?: IFeatureController
  ): number {
    let val = base;
    if (!source) return base;
    const replace = source.FeatureController.Bonuses.filter(
      (x) => x.ID === id && x.Replace
    );
    if (replace.length)
      val = replace.reduce(
        (sum, bonus) => sum + this.Evaluate(bonus, source),
        0
      );
  ...
  public static get(id: string, source: IFeatureController): number {
    let bArr = source.FeatureController.Bonuses.filter(
      (x) => x.ID === id && !x.Replace
    );
  ...
  public static Evaluate(bonus: Bonus, source: IFeatureController): number {
    ...
    const ll =
      source.FeatureController.getRootProperty('Level')?.toString() || '0';
    const grit =
      source.FeatureController.getRootProperty('Grit')?.toString() || '0';
  ...
  public static Contributors(
    ...
    return controller.FeatureController.Bonuses.filter((x) => x.ID === id).map(
      (b) => ({
        name: b.Source,

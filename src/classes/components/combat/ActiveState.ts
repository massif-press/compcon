...
import {
  IDeployableData,
  IDeployedData,
} from '../feature/deployable/Deployable';
...
class ActiveState {
  ...
  public constructor(pilot: PilotInstance) {
    this._mech = null;
  ...
  public get IsProtocolAvailable(): boolean {
    return (
      this.Move === this.MaxMove && this.Actions === 2 && !this._overcharged
    );
  }
  ...
  public get IsSkirmishAvailable(): boolean {
    return (
      this.Actions > 0 &&
      !this.AllActions.find((x) => x.ID === 'act_skirmish').Used
    );
  }
  ...
  public UndoEndTurn(): void {
    ...
    if (this._cachedBurnStructure)
      this.ActiveMech.CurrentStructure += this._cachedBurnStructure;
    this.InTurn = true;
  }
  ...
  public ResetActions(event: ActivePeriod): void {
    ...
    if (event == ActivePeriod.Scene || event == ActivePeriod.Mission) {
      if (this.ActiveMech.Frame.CoreSystem.PassiveActions)
        this.ActiveMech.Frame.CoreSystem.PassiveActions.forEach((a) =>
          a.Reset(event)
        );
      if (this.ActiveMech.Frame.CoreSystem.DeployActions)
        this.ActiveMech.Frame.CoreSystem.DeployActions.forEach((a) =>
          a.Reset(event)
        );
    }
  }
  ...
  public NextRound(): void {
    ...
    this.ActiveMech.Pilot.PilotLoadoutController.Loadout.Equipment.forEach(
      (e) => e.Reset()
    );
  ...
  public StartRest(): void {
    ...
    if (this.ActiveMech.Pilot.IsDownAndOut)
      this.ActiveMech.Pilot.CurrentHP = Math.ceil(
        this.ActiveMech.Pilot.MaxHP / 2
      );
    this.SetLog({
      id: 'start_combat',
      event: 'LOG.END',
  ...
  RepairStructure(): void {
    ...
    const cheap = this.ActiveMech.FeatureController.Bonuses.some(
      (x) => x.ID === 'cheap_struct'
    );
  ...
  public get AvailableAmmoUses(): number {
    const ac =
      this.ActiveMech.MechLoadoutController.ActiveLoadout.IntegratedSystems.find(
        (x) => x.ID.includes('walking_armory')
      );
    if (!ac) return 0;
    else return ac.Uses;
  }
  ...
  public SetLog(
    entry: { id: string; event: string; detail: string },
    action?: Action
  ) {
    this._log.push({
      id: entry.id,
  ...
  public CommitAction(action: Action, free?: boolean) {
    ...
    if (!free) {
      ...
      if (act === ActivationType.Quick || act === ActivationType.QuickTech)
        activationCost = 1;
      else if (act === ActivationType.Full || act === ActivationType.FullTech)
        activationCost = 2;
    }
  ...
  public SetMove(val: number) {
    ...
    this.SetLog({
      ...
      detail: `${
        val > 0
          ? `FRAME/COMMIT.TAC: ${val} SPACES`
          : `FRAME/RESCIND.TAC: ${Math.abs(val)} SPACES`
      }`,
    });
  }
  ...
  public SetStatusCondition(statuses: string[], isStatus?: boolean) {
    ...
    this.SetLog({
      ...
      detail: `${
        added ? '' : '!ALERT! '
      }FRAME/STATUS.${sstr.toUpperCase()} ++ALARM.${removed ? 'OFF' : 'ON'}++`,
    });
  }
  ...
  public SetResistance(resistances: string[]) {
    ...
    const added = resistances.find(
      (x) => !this.ActiveMech.Resistances.includes(x)
    );
    const removed = this.ActiveMech.Resistances.find(
      (x) => !resistances.includes(x)
    );
    const sstr = added ? added : removed;
    this.ActiveMech.Resistances = resistances;
    this.SetLog({
      id: `set_res`,
      event: 'RESISTANCE',
      detail: `FRAME/DEF.RES::${sstr.toUpperCase()}${
        removed ? '++RES.END++' : ''
      }`,
    });
  }
  ...
  public SetStructure(val: number) {
    ...
    const pct = (
      this.ActiveMech.CurrentStructure / this.ActiveMech.MaxStructure
    ).toFixed(2);
    this.SetLog({
      id: `set_str`,
  ...
  public SetRepCap(val: number) {
    ...
    this.SetLog({
      ...
      detail: `${
        val < 0
          ? `FRAME/COMMIT.REPAIR: ${val}`
          : `FRAME/RECOVER.REPAIR: ${Math.abs(val)}`
      }`,
    });
  }

  public SetCorePower(val: number) {
    if (this.ActiveMech.CurrentCoreEnergy > 0 && val === 0)
      this.ActiveMech.IsCoreActive = true;
    ...
    this.SetLog({
      ...
      detail: `${
        val > 0
          ? `FRAME/CORE:: CAPACITY RESTORED`
          : `!ALERT! FRAME CORE ACTIVATION !ALERT!`
      }`,
    });
  }
  ...
  public CommitStabilize() {
    ...
    if (this.StabilizeMinor === 'reload') {
      ...
      str += ' ::SYS.RESTORE';
    else if (this.StabilizeMinor === 'end_ally_condition')
      str += ' ::REMOTE.ASSIST';
  ...
  public CommitOvercharge() {
    ...
    if (
      this.ActiveMech.CurrentOvercharge < this.ActiveMech.OverchargeTrack.length
    )
  ...
  public UndoOvercharge() {
    ...
    if (this.ActiveMech.CurrentOvercharge > 0)
      this.ActiveMech.CurrentOvercharge -= 1;
  ...
  public UndoShutDown() {
    ...
    this.ActiveMech.MechLoadoutController.ActiveLoadout.Equipment.forEach(
      (e) => {
  ...
  public Deploy(d: IDeployableData) {
    ...
    this.SetLog({
      ...
      detail: `FRAME/REMOTE::${d.name
        .toUpperCase()
        .replace(/\s/g, '.')}.${n} ++STATUS OK++`,
    });
    this.save();
  }
  ...
  public get AvailableActions(): string[] {
    ...
    if (!this.IsMounted) {
      const pilotActions = this.AllActions.filter(
        (x) => x.IsPilotAction && !x.IsActiveHidden
      ).map((x) => x.ID);
      ...
      const out = this.AllActions.filter(
        (x) => x.IsMechAction && !x.IsActiveHidden
      ).map((x) => x.ID);
      if (!this.ActiveMech.IsShutDown)
        out.splice(out.indexOf('act_boot_up'), 1);
      return out;
    }
  }

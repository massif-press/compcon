...
class ActiveStatController {
  ...
  public set CurrentHP(val: number) {
    if (val > this.Parent.StatController.MaxHP) this._currentHP = this.Parent.StatController.MaxHP
    else if (val <= 0) {
  ...
  public set CurrentHeat(val: number) {
    if (val > this.Parent.StatController.HeatCapacity) {
      ...
      this.Parent.StatController.HeatCapacity = val - this.Parent.StatController.HeatCapacity
  ...
  public set CurrentOvercharge(overcharge: number) {
    ...
    if (this._currentOvercharge > this.Parent.StatController.OverchargeTrack.length - 1)
      this._currentOvercharge = this.Parent.StatController.OverchargeTrack.length - 1
  }
  ...
  public get IsDangerZone(): boolean {
    return this._currentHeat >= Math.ceil(this.Parent.StatController.HeatCapacity / 2)
  }
  ...
  public static Serialize(parent: IStatContainer, target: any) {
    ...
    target.current_stats.currentStructure = parent.ActiveStatController.CurrentStructure
    ...
    target.current_stats.currentStress = parent.ActiveStatController.CurrentStress
    target.current_stats.currentRepairs = parent.ActiveStatController.CurrentRepairs
    ...
    target.current_stats.currentOvercharge = parent.ActiveStatController.CurrentOvercharge
    ...
    target.current_stats.isBraceCooldown = parent.ActiveStatController.IsBraceCooldown
    target.current_stats.isOvercharged = parent.ActiveStatController.IsOvercharged
  }
...
export { ActiveStatController, IActiveStatData }

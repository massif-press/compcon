...
class WeaponMod extends MechEquipment {
  ...
  public constructor(
    ...
    this.AllowedTypes =
      data.allowed_types ||
      Object.keys(WeaponType).map((k) => WeaponType[k as string]);
    this.AllowedSizes =
      data.allowed_sizes ||
      Object.keys(WeaponSize).map((k) => WeaponSize[k as string]);
    ...
    this.AddedDamage = data.added_damage
      ? data.added_damage.map((x) => new Damage(x))
      : [];
    this.AddedRange = data.added_range
      ? data.added_range.map((x) => new Range(x))
      : [];
  ...
  public get PossibleTypes(): WeaponType[] {
    return this.AllowedTypes.filter(
      (x) =>
        !this.RestrictedTypes.some((y) => y.toLowerCase() === x.toLowerCase())
    );
  }
  ...
  public get PossibleSizes(): WeaponSize[] {
    return this.AllowedSizes.filter(
      (x) =>
        !this.RestrictedSizes.some((y) => y.toLowerCase() === x.toLowerCase())
    );
  }

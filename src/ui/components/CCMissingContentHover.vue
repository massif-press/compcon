<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="controller && controller.NonfunctionalBrews.length"
        class="pa-1"
      >
      ...
      >
        <i18n-t
          v-if="itemType.toLowerCase === 'eidolon'"
          keypath="ui.missing.eidolonHelp"
          tag="span"
          scope="global"
        >
          <template #pack>
            <strong>{{ $t('ui.missing.wallflowerPack') }}</strong>
          </template>
        </i18n-t>
        <i18n-t
          v-else
          keypath="ui.missing.npcHelp"
          tag="span"
          scope="global"
        >
          <template #pack>
            <strong>{{ $t('ui.missing.lancerCoreBook') }}</strong>
          </template>
        </i18n-t>
      </v-card-text>
    </v-card>

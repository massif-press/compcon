<template>
  <v-card>
    ...
    <div v-else>
      ...
      >
        <v-col
          v-for="image in displayedUserImages"
          :key="image.uri"
          cols="4"
          md="3"
        >
          ...
          <v-scale-transition>
            ...
            >
              <div class="text-caption pb-1 text-center">
                {{ image.name }} &mdash; {{ (image.size / 1024 / 1024).toFixed(2)
                }}{{ $t('ui.image.mb') }}
              </div>
              <v-menu
                offset-y
      ...
      <v-card-text>
        ...
        <div>
          ...
          <v-progress-linear
            :value="(accountUsage / accountMax) * 100"
            height="20px"
          >
            <template #default>
              <strong>
...
<script setup lang="ts">
  ...
  async function uploadImage() {
    ...
    notify({
      ...
      text: t('notify.image.uploadedText', {
        file: `${filename}.${ext}`,

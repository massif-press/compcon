<template>
  ...
  >
    <div v-show="item.Profiles.length > 1">
      ...
      >
        <v-chip
          v-for="(p, index) in item.Profiles"
          :key="p.Name"
          :value="index"
          size="small"
          filter
    ...
    >
      <div>
        ...
        >
          ...
          <p
            v-html-safe="item.Profiles[tab].Effect"
            class="px-2"
          />
        </div>
      </div>
      ...
      <div v-if="item.Profiles[tab].Actions.length">
        ...
        >
          <v-col
            v-for="(a, index) in item.Profiles[tab].Actions"
            :key="`action-${index}`"
            cols="auto"
          >
            <cc-action
      ...
      <div v-if="item.Profiles[tab].Deployables.length">
        ...
        >
          <v-col
            v-for="(d, i) in item.Profiles[tab].Deployables"
            :key="`deployable-${i}`"
            cols="auto"
          >
            <cc-deployable-info

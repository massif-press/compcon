<template>
  ...
  >
    ...
    >
      <v-col cols="auto">
        <cc-range-element
          :range="item.Range(tier || 1)"
          :small="dense"
        />
      </v-col>
      <v-col
        v-if="item.Damage(0).length"
        cols="auto"
        :class="dense ? '' : 'mt-n2'"
      >
        ...
        >
          ...
          >
            ...
            <span
              ...
              style="font-size: inherit; font-weight: 100"
            >
              /
            </span>
          </v-col>
    ...
    <template #statblock>
      <p
        v-if="item.OnMiss"
        v-html-safe="`<b>On Miss:&nbsp;</b>${item.OnMiss.getDetail(tier)}`"
      ...
      <p
        v-if="item.OnHit"
        v-html-safe="`<b>On Hit:&nbsp;</b>${item.OnHit.getDetail(tier)}`"
        class="panel text-text py-1"
      />
      <p
        v-if="item.OnCrit"
        v-html-safe="`<b>On Crit:&nbsp;</b>${item.OnCrit.getDetail(tier)}`"
        class="panel text-text py-1"
      />
    </template>
  </equipment-card-base>

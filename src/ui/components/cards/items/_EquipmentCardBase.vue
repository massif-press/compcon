<template>
  <div>
    ...
    <div v-if="item.Effect">
      <div class="text-overline ml-n2 text-subtle">EQUIPMENT EFFECT</div>
      <p v-html-safe="item.Effect" class="text-text body-text mb-1" />
    </div>

    <div v-if="item.Actions && item.Actions.length">
      <div class="text-overline ml-n2 text-subtle">EQUIPMENT ACTIONS</div>
    ...
    <div v-if="item.IntegratedEquipment && item.IntegratedEquipment.length">
      <div class="text-overline ml-n2 text-subtle">EQUIPMENT INTEGRATIONS</div>
      <v-row no-gutters justify="center">
        <v-col v-for="(x, i) in item.IntegratedEquipment" cols="auto">

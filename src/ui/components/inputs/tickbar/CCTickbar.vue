...
<script setup lang="ts">
  ...
  function setTertiaryVal(val: number) {
    ...
    if (val < 0) val = 0
    if (val > Math.min(props.max || 100, props.tertiaryTicks || 100))
      val = Math.min(props.max || 100, props.tertiaryTicks || 100)
    emit('update:tertiary', val)
  }
  function setSecondaryVal(val: number) {
    if (props.stopAdd && val > Number(props.secondary)) return
    if (val < 0) val = 0
    if (val > Math.min(props.max || 100, props.secondaryTicks || 100))
      val = Math.min(props.max || 100, props.secondaryTicks || 100)

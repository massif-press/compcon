      ...
      <v-container v-if="!confirmed" fluid>
          ...
          <v-col lg="auto" cols="12" class="mt-n5">
            ...
            >
              <v-col
                v-if="item.Range"
                cols="auto"
                :class="$vuetify.display.mdAndUp ? 'mr-8' : ''"
              >
            ...
            >
              ...
              <v-col v-if="aux" cols="auto" align-self="center">
                <v-btn
                  ...
                  block
                  :disabled="!attackRoll"
                  :color="`${crit ? 'secondary' : 'action--free'} ${
                    attackFree ? 'lighten-1' : ''
                  }`"
                  @click="attackFree = !attackFree"
                >
                  <v-icon start>cc:free-action</v-icon>
                  Attack
                </v-btn>
              </v-col>
              <v-col v-else-if="overwatch" cols="auto" align-self="center">
                <v-btn
                  ...
                  @click="attackFree = !attackFree"
                >
                  <v-icon start>cc:reaction</v-icon>
                  Attack
                </v-btn>
              </v-col>
              <v-col v-else cols="auto" class="ml-2 mt-n1">
                <v-btn
                  ...
                  :disabled="
                    attackFree ||
                    !attackRoll ||
                    (!improv && !state.IsSkirmishAvailable)
                  "
                  :color="`${
                    crit
                      ? 'secondary'
        ...
        <v-slide-x-reverse-transition>
          <v-row
            v-if="hit || missed"
            align="center"
            :class="`mt-1 ${$vuetify.display.smAndDown ? 'panel' : ''}`"
          >
            <v-col cols="auto" class="ml-auto" />
...
each source of damage is used.`"
            ...
            <v-slide-x-reverse-transition>
              <v-col
                ...
                cols="auto"
                :class="
                  $vuetify.display.mdAndUp ? 'px-12 panel dual-sliced' : ''
                "
                style="height: 70px"
              >
      ...
      <v-row v-if="(hit || missed) && !noDamageItem" no-gutters class="mt-2">
        <v-col cols="auto" class="ml-auto">
          <p class="flavor-text text-stark ma-0">
            ...
            <span v-if="hit">
              {{ improv ? 'Improvised attack' : 'Weapon activation' }}
              registered. {{ crit ? 'Direct hit' : 'Hit' }} confirmed.
            </span>
...
export default {
  ...
  computed: {
    ...
    overkill() {
      ...
      if (this.item.Mod && this.item.Mod.AddedTags.some((x) => x.IsOverkill))
        return true;
      return false;
    },
    crit() {
    ...
    hardpoints() {
      ...
      return (
        this.mount.Bonuses &&
        this.mount.Bonuses.find(
          (x) => x.ID === 'cb_auto_stabilizing_hardpoints'
        )
      );
    },
    overpower() {
      ...
      return (
        this.mount.Bonuses &&
        this.mount.Bonuses.find((x) => x.ID === 'cb_overpower_caliber')
      );
    },
    armoryLevel() {
      if (

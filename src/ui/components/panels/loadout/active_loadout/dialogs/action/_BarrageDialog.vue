...
export default {
  ...
  methods: {
    disableShBarrage(w) {
      ...
      return (
        w.Used ||
        this.state.BarrageSelections.length > 0 ||
    ...
    setSHBarrage(item, mount) {
      ...
      const self = this;
      Vue.nextTick().then(() =>
        Vue.nextTick().then(() => self.$refs.sh_b_dialog.show())
      );
    },
    completeBarrage() {
      this.$emit('use');

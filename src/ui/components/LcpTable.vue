  ...
  >
    ...
    <template #item.local_version="{ item }">
      <span v-if="getInstalledPack(item)">
        ...
        <v-icon
          v-if="item.version === getInstalledPack(item)?.Manifest.version"
          color="success"
        >
          mdi-check-bold
        </v-icon>
...
<script setup lang="ts">
  ...
  function getInstalledPack(pack: any) {
    return contentPacks.value.find(({ Manifest }) =>
      [pack.name, pack.title].some((p: string) =>
        Manifest.name?.toLowerCase().includes(p?.toLowerCase())
      )
    )
  }

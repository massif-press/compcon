<template>
  ...
  >
    ...
    >
      ...
      >
        <v-alert
          v-show="hasSlotContent($slots.top)"
          ref="topEl"
          variant="outlined"
          class="mb-3 py-1"
          style="
        ...
        <div v-if="view === 'list' && bprops.itemType === 'License'">
          <v-row
            v-for="m in facets.manufacturerSources.value"
            :key="`mf-list-${m}`"
          >
            <v-col
              v-if="!isFallbackGroup(m)"
            ...
            >
              ...
              >
                <license-expandable
                  :items="<License[]>facets.getItems(m)"
                ...
                $slots.item
                  ? [
                      ...
                      activeIds.includes(item.ID)
                        ? { backgroundColor: 'rgba(var(--v-theme-primary), 0.15)' }
                        : {},
                    ]
                  : {}
              "
...
<script setup lang="ts">
  import { computed, ref, watch, inject, nextTick, onMounted, onBeforeUnmount } from 'vue'
  ...
  function getMultiHeader(subtype: string) {
    ...
    return bprops.multiHeaders && bprops.multiHeaders[key]
      ? bprops.multiHeaders[key]
      : bprops.tableHeaders
  }
  ...
  const tableGroups = computed(() => {
    ...
    switch (group.value) {
      case 'lcp':
        return map(facets.filteredLcps.value, k => facets.filteredItemsByLcp.value[k], {
      ...
      case 'type':
        return facets.subtypes.value.map(k => ({
          ...
          headClass: accent,
          logo: null,
          headers: getMultiHeader(k),
          items: facets.itemsByType.value[k],
        }))
      case 'role':
  ...
  function measureContentHeight() {
    ...
    const bottom =
      bprops.pageScroll && mobile.value
        ? window.innerHeight - MOBILE_FOOTER_RESERVE
        : overlay
          ? overlay.getBoundingClientRect().bottom
          : window.innerHeight
    const minBottom =
      bprops.pageScroll && !mobile.value ? rect.top + window.innerHeight * MIN_FILL : 0
    const edge = Math.max(bottom, minBottom)
    contentHeight.value = Math.max(0, Math.floor(edge - rect.top))

    const listEl: HTMLElement | undefined = listScroller.value?.$el
    if (listEl) {
      const listTop = listEl.getBoundingClientRect().top
      listHeight.value = Math.max(0, Math.floor(edge - listTop - 16))
    } else {
      listHeight.value = Math.max(0, contentHeight.value - 32)
    }
  }

  function scrollTo(id: string, container?: HTMLElement): void {
    const el = document.getElementById(id)
    const target: HTMLElement | undefined = container ?? mainEl.value?.$el ?? mainEl.value
    if (!el || !target) return
    const elRect = el.getBoundingClientRect()
    const cRect = target.getBoundingClientRect()
    const y =
      elRect.top - cRect.top + target.scrollTop - target.clientHeight / 2 + elRect.height / 2
  ...
  async function scrollVirtualTo(idx: number, id: string) {
    const scroller = listScroller.value
    const container: HTMLElement | undefined = scroller?.$el
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (el && container.contains(el)) {
        scrollTo(id, container)
        return
      }
    }
  }

  async function revealItem(item: CompendiumItem | License) {
    const idx = facets.navOrderedItems.value.findIndex((x: any) => x.ID === item.ID)
    if (idx < 0) return
    if (view.value === 'list') {
      if (useVirtualScroll.value) await scrollVirtualTo(idx, item.ID)
  ...
  watch(
    () => facets.shownItems.value.map((i: any) => i.ID).join(),
    () => {

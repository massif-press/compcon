        ...
        <div>
          ...
          >
            <div v-if="item.Prerequisite.cumulative">
              {{
                $t('ui.license.prereqCumulative', {
                  rank: item.Prerequisite.min_rank,
                  source: item.Prerequisite.source,
                })
              }}
            </div>
            <div v-else>
              {{
                $t('ui.license.prereqSingle', {
                  source: item.Prerequisite.source,
                  rank: item.Prerequisite.min_rank,
                })
              }}
          ...
          >
              ...
              >
                ...
                >
                  <span v-if="getControllerRank(item)">
                    {{ $t('common.remove') }} {{ item.Name }}
                    {{ 'I'.repeat(getControllerRank(item)) }}
                  </span>
                  <span v-else>
            ...
            >
              <cc-button
                ...
                color="success"
                :disabled="
                  !controller.IsMissingLicenses || getControllerRank(item) >= item.Unlocks.length
                "
                prepend-icon="mdi-plus"
                @click="$emit('add', item)"
              >
                <span v-if="getControllerRank(item) < item.Unlocks.length">
                  {{ $t('common.unlock') }} {{ item.Name }}
                  {{ 'I'.repeat(getControllerRank(item) + 1) }}
                </span>
                <span v-else>

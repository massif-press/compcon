<template>
  <v-layout class="browser-layout">
    ...
    >
      ...
      >
        <v-alert
          v-show="hasSlotContent($slots.header)"
          variant="outlined"
          class="mb-3 py-1"
          tile
...
<script setup lang="ts">
  ...
  const searchRaw = ref('')
  const search = ref('')
  const _debouncedSetSearch = _.debounce((val: string) => {
    search.value = val
  }, 200)
  ...
  watch(search, val => {
    if (val) {
      ...
      for (const lcp of curLcps) {
        ...
        _.uniq(lcpItems.map(sourceGroup).filter(Boolean)).forEach((mfId: string) =>
          subGroups.push(`${lcp}_${mfId}`)
        )
        _.uniq(lcpItems.filter((x: any) => x.Role).map((x: any) => x.Role)).forEach(
          (role: string) => subGroups.push(`${lcp}_${role}`)
        )
        _.uniq(lcpItems.filter((x: any) => x.Origin?.Name).map((x: any) => x.Origin.Name)).forEach(
          (origin: string) => subGroups.push(`${lcp}_${origin}`)
        )
      }
      open.value = [
        ...curLcps,
        ...subGroups,
        ...manufacturerSources.value,
        ...subtypes.value,
        ...licenses.value,
        ...roles.value,
        ...origins.value,
  ...
  function revealItem(item: CompendiumItem | License) {
    contentRef.value?.revealItem(item)
  }
  ...
  const _mfMap = computed(
    () => new Map((props.manufacturers as Manufacturer[]).map(m => [m.ID, m]))
  )

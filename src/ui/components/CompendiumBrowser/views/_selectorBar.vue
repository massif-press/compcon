<template>
      ...
      >
        ...
        <v-row class="mt-n8">
          <v-col style="height: calc(100vh - 225px)">
            <bar
              :data="getChartData(getItems(manufacturer))"
              :options="options"
            />
          </v-col>
        </v-row>
  ...
  <div v-else-if="group === 'license'">
    ...
    <v-window v-model="licenseTab">
      ...
      >
        <div
          class="heading h2 text-primary mt-4"
          v-text="groupLabel(l)"
        />
        <v-row class="mt-n8">
          <v-col style="height: calc(100vh - 225px)">
            <bar
              :data="getChartData(getLicenseItems(l as string))"
              :options="options"
            />
          </v-col>
        </v-row>
...
<script setup lang="ts">
  ...
  function sortItems(items: any[]) {
    ...
    if (sort.value === 'descending')
      return _.sortBy(items, (x: any) => x.stats[xAxis.value.value]).reverse()
    return items
  }

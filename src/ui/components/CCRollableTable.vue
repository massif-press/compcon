<template>
  <v-card flat>
    <v-alert outlined style="border-width: 3px !important" class="pa-2" :color="color">
      <v-card-text class="pa-0 ma-0">
        ...
        <v-row v-else dense align="center">
          ...
          <v-col cols="auto" class="mt-n2">
            <v-select v-model="table.Mult" dense hide-details style="width: 50px" :items="mults" />
          </v-col>
          <v-col cols="auto">
            <span class="heading h3 grey--text">D</span>
          </v-col>
          <v-col cols="auto" class="mt-n2">
            <v-select v-model="table.Die" dense hide-details style="width: 50px" :items="dice" />
          </v-col>
          <v-col cols="auto" class="heading h4 grey--text">Step</v-col>
          <v-col cols="1" class="mt-n2">
            <v-text-field v-model="step" type="number" style="width: 50px" dense hide-details />
          </v-col>
          <v-col cols="auto">
            <v-menu offset-x bottom>
              <template v-slot:activator="{ on }">
                <v-btn small color="primary" class="fadeSelect" v-on="on">

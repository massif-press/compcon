  ...
  >
    <v-col cols="auto">
      ...
      >
        <template #reason>
          <div
            v-if="controller.IsActionUsed(action.ID)"
            class="text-center"
          >
            {{ $t('ui.combat.actionUsed') }}
          </div>
          <div
            v-else-if="!controller.CanActivate(action.Activation)"
            class="text-center"
          >
            <i18n-t
              keypath="ui.combat.insufficientActions"
              tag="span"
              scope="global"
            >
              <template #activation>
...
<script setup lang="ts">
  ...
  const canActivate = computed(
    (): boolean =>
      !props.disabled &&
      controller.value.CanActivate(props.action.Activation) &&
      !controller.value.IsActionUsed(props.action.ID)
  )

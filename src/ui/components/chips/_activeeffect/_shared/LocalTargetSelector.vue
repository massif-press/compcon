<template>
  ...
  >
    ...
    >
      ...
      <v-col>
        <v-btn-toggle
          v-if="idx === 0 && event.TargetType === 'self'"
        ...
        <v-card
          ...
          tile
        >
          <template v-if="event.AoE">{{ getOrdinal(Number(idx) + 1) }}</template>
          {{ $t('ui.combat.target') }}
          <v-btn
            v-if="event.Targets?.length > 1"
            icon
            size="20"
            variant="text"
            class="mr-n2"
        ...
        <v-btn
          v-if="event.AoE && idx === event.Targets?.length - 1"
          size="x-small"
          block
          flat
          tile

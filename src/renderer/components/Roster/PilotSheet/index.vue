        ...
        <div v-if="pilot.name">
            <v-container fluid>
                <v-layout>
                        ...
                        <v-layout>
                            <v-flex>
                                ...
                                <v-rating
                                    ...
                                    hover
                                    x-large
                                    :length="stats.hp + stats.armor"
                                    readonly
                                    small
                                    dense
                                    empty-icon="brightness_7"
                    ...
                    <v-flex shrink>
                        <span class="caption" style="float:right; text-align: right">LICENSE LEVEL</span>
                        <br>
                    </v-flex>
                    <span
                        style="font-size: 120px; line-height: 90px; float:right; font-weight:100"
                    >{{pilot.level}}</span>
                    <v-flex shrink>
                        ...
                        <v-dialog v-model="levelEditor" width="900">
                            <v-card>
                                            ...
                                            >
                                                <v-card-text>
                                                    <v-text-field
                                                        ...
                                                        @click:append-outer="levelSkip ++"
                                                        prepend-icon="remove"
                                                        @click:prepend="levelSkip --"
                                                        class="pb-0 mb-0"
                                                    />
                                                </v-card-text>
                                            </v-card>
                                ...
                                <v-layout>
                                    <v-flex>
                                        <v-alert
                                            :value="levelSkip && levelSkip < pilot.level"
                                            type="warning"
                                        >
                                            <b>WARNING: LEVEL REMOVAL</b>
                                            <br>This will reset all Pilot Skill Triggers, Talents, Licenses, Mech Skills, and CORE Bonuses. They must be re-added manually.
                                        </v-alert>
                                    </v-flex>
                                </v-layout>
                                <v-divider/>
                ...
                <v-layout>
                    <v-flex xs8>
                        <v-layout>
                            <v-flex>
                                ...
                                <v-layout>
                                    <v-flex xs12 style="text-align:center">
                                        ...
                                        <div v-else style="display: inline">
                                            <span
                                                v-if="item('Backgrounds', pilot.background).err"
                                                class="grey--text"
                                            >// MISSING BACKGROUND DATA //</span>
                                            <b
                                                v-else
                                            >{{ item('Backgrounds', pilot.background).name }}</b>
                                        </div>
                                        <v-dialog
                                            lazy
                                ...
                                <v-layout wrap class="ml-3 mr-3">
                                    <v-flex
                                        shrink
                                        v-for="(invoke, index) in pilot.invocations"
                                        :key="invoke.trigger"
                                    >
                                        <invocation-item
                                            :invoke="invoke"
                                            :index="index"
                                            @remove-invoke="removeInvocation"
                                        />
                                    </v-flex>
                                    <v-flex>
                                        <v-dialog v-model="invokeDialog" width="600">
                                            ...
                                            <v-card>
                                                <v-card-title
                                                    class="title"
                                                >Add Background Invocation</v-card-title>
                                                <v-card-text>
                                                    <v-text-field
                                                        v-model="invoke_trigger"
                                                        label="Invocation Trigger"
                                                        outline
                                                    />
                                                    <v-flex class="text-xs-center">
                                                        <v-btn-toggle
                                                            v-model="invoke_attribute"
                                                            dark
                                                        >
                                                            <v-btn large color="primary">
                                                                <v-icon small>thumb_up</v-icon>&emsp;
                                                                <span>Accuracy</span>
                                                            </v-btn>
                                                            <v-btn large color="error">
                                                                <v-icon small>thumb_down</v-icon>&emsp;
                                                ...
                                                <v-card-actions>
                                                    <v-btn
                                                        color="primary"
                                                        flat
                                                        @click="invokeDialog = false"
                                                    >Cancel</v-btn>
                                                    <v-spacer/>
                                                    <v-btn
                                                        color="primary"
                                                        :disabled="invoke_trigger === '' || (!invoke_attribute && invoke_attribute !== 0)"
                                                        @click="addInvocation"
                                                    >Add New Invocation</v-btn>
                                                </v-card-actions>
                                            </v-card>
                                        </v-dialog>
                                    </v-flex>
                        ...
                        <v-layout>
                            <v-flex>
                                <v-layout>
                                    <span class="header no-icon">Contacts</span>
                                </v-layout>
                                <contacts-list
                                    :contacts="pilot.contacts"
                                    @add-contact="refresh"
                                    :key="pilot.contacts.length"
                                />
                            </v-flex>
                        </v-layout>
                    </v-flex>
                        ...
                        <v-layout>
                            <span class="header">
                                ...
                                <v-btn
                                    ...
                                    icon
                                    color="primary"
                                    @click="appearanceModal = true; appearanceLoader = true;"
                                >
                                    <v-icon small>edit</v-icon>
                                </v-btn>
                            </span>
                        ...
                        <v-layout>
                            ...
                            >
                                <div v-if="pilot.portrait">
                                    <v-img
                                        :src="`file://${userDataPath}/img/portrait/${pilot.portrait}`"
                                        max-height="55vh"
                                        max-width="45.1vw"
                                        contain
                                    />
                                ...
                                >
                                    <v-card>
                                        <v-toolbar fixed dense flat>
                                            ...
                                            <v-toolbar-items>
                                                <v-btn
                                                    icon
                                                    large
                                                    @click="appearanceModal = false; appearanceLoader = false"
                                                >
                                                    <v-icon large>close</v-icon>
                                                </v-btn>
                                            </v-toolbar-items>
                                        ...
                                        <div v-if="appearanceLoader">
                                            <image-selector
                                                :preselectPortrait="pilot.portrait"
                                                :preselectAvatar="pilot.avatar"
                                                @assign-portrait="setPortrait"
                                                @assign-avatar="setAvatar"
                                            />
                                        </div>
                        ...
                        <v-layout>
                            <v-flex class="pl-2">
                                <editable-textfield
                                    ...
                                    :attr="'text_appearance'"
                                    :initial="pilot.text_appearance"
                                    :key="pilot.id"
                                />
                            </v-flex>
                        </v-layout>
                ...
                <v-layout>
                    <v-flex xs1>
                        <v-layout>
                            <span class="header no-icon">Grit</span>
                        </v-layout>
                        <v-layout align-center justify-center column fill-height>
                            <v-flex>
                                <span
                                    class="display-3 font-weight-black text-xs-center"
                                >+{{stats.grit}}</span>
                            </v-flex>
                        </v-layout>
                    </v-flex>
                                        ...
                                        <v-toolbar fixed dense flat>
                                            ...
                                            <v-toolbar-items>
                                                <v-btn
                                                    icon
                                                    large
                                                    @click="skillModal = false; skillLoader = false"
                                                >
                                                    <v-icon large>close</v-icon>
                                                </v-btn>
                                            </v-toolbar-items>
                        ...
                        >
                            ...
                            <v-card>
                                <v-toolbar fixed dense flat>
                                    ...
                                    <v-spacer></v-spacer>
                                    <v-toolbar-items>
                                        <v-btn icon large @click="licenseModal = false">
                                            <v-icon large>close</v-icon>
                                        </v-btn>
                                    </v-toolbar-items>
                                </v-toolbar>
                                ...
                                <div v-if="licenseLoader" class="mt-5">
                                    <license-selector
                                        ref="licenseSelector"
                                        :pilotLicenses="pilot.licenses"
                                        :pilotLevel="pilot.level"
                                        @set-licenses="setLicenses"
                                    />
                                </div>
                                <v-layout justify-space-between>
                                    <v-flex xs1>&emsp;</v-flex>
                                    <v-flex xs1>
                                        <v-btn
                                            color="primary"
                                            flat
                                            @click="licenseModal = false; licenseLoader = false"
                                        >Confirm</v-btn>
                                    </v-flex>
                                </v-layout>
                            </v-card>
                        ...
                        >
                            ...
                            <v-card>
                                <v-toolbar fixed dense flat>
                                    ...
                                    <v-toolbar-items>
                                        <v-btn
                                            icon
                                            large
                                            @click="talentModal = false; talentLoader = false"
                                        >
                                            <v-icon large>close</v-icon>
                                        </v-btn>
                                    </v-toolbar-items>
                                ...
                                <div v-if="talentLoader">
                                    <talent-selector
                                        ref="talentSelector"
                                        :pilotTalents="pilot.talents"
                                        :pilotLevel="pilot.level"
                                        @set-talents="setPilotTalents"
                                    />
                                </div>
                            </v-card>
                        </v-dialog>
                ...
                <div class="ml-3 mr-3">
                    <v-expansion-panel focusable>
                        <talent-item
                            ...
                            :key="talent.id"
                            :talent="talent"
                            :talentData="item('Talents', talent.id)"
                        />
                    </v-expansion-panel>
                </div>
                <v-layout>
                                ...
                                <v-toolbar fixed dense flat>
                                    ...
                                    <v-toolbar-items>
                                        <v-btn
                                            icon
                                            large
                                            @click="mechSkillModal = false; mechSkillLoader = false"
                                        >
                                            <v-icon large>close</v-icon>
                                        </v-btn>
                                    </v-toolbar-items>
                ...
                <v-layout center justify-space-around class="pl-5">
                    <v-flex>
                        HULL
                        <span class="grey--text">({{pilot.mechSkills.hull}})</span>
                        <v-rating/>
                    </v-flex>
                    <v-flex xs3>
                        AGILITY
                        <span class="grey--text">({{pilot.mechSkills.agi}})</span>
                        <v-rating/>
                    </v-flex>
                    <v-flex xs3>
                        SYSTEMS
                        <span class="grey--text">({{pilot.mechSkills.sys}})</span>
                        <v-rating/>
                    </v-flex>
                    <v-flex xs3>
                        ENGINEERING
                        <span class="grey--text">({{pilot.mechSkills.eng}})</span>
                        <v-rating/>
                    </v-flex>
                </v-layout>
                <v-layout>
                        ...
                        >
                            ...
                            <v-card>
                                <v-toolbar fixed dense flat>
                                    ...
                                    <v-toolbar-items>
                                        <v-btn
                                            icon
                                            large
                                            @click="bonusModal = false; cbLoader = false"
                                        >
                                            <v-icon large>close</v-icon>
                                        </v-btn>
                                    </v-toolbar-items>
                                ...
                                <div v-if="cbLoader">
                                    <core-bonus-selector
                                        ref="cbSelector"
                                        :pilotBonuses="pilot.core_bonuses"
                                        :pilotLevel="pilot.level"
                                        :pilotLicenses="pilot.licenses"
                                        @set-bonuses="setPilotBonuses"
                                    />
                                </div>
                            </v-card>
                        </v-dialog>
                ...
                <v-layout>
                    <span class="header no-icon">Pilot Gear</span>
                </v-layout>
                <v-layout>
                    <v-flex xs12>
                        <pilot-loadout/>
                ...
                <v-layout>
                    <span class="header no-icon">Notes</span>
                </v-layout>
                <v-layout>
                    <v-flex>
                        <editable-textfield
                ...
                <v-layout justify-center>
                    <v-flex xs10>
                        <v-alert :value="!pilot.configs.length" color="info" icon="info" outline>
                            ...
                            <br>This pilot does not have any mech Configurations associated with thier profile. A new Configuration can be added by clicking "Add New Configuration" under the Pilot's entry on the sidebar, or, by &nbsp;
                            <v-btn
                                small
                                color="info"
                                outline
                                class="ma-0 pa-1"
                                @click="newConfigModal = true; newConfigLoader = true"
                            >clicking here</v-btn>
                        </v-alert>
                        <v-dialog
                            lazy
                            v-model="newConfigModal"
                            fullscreen
                            hide-overlay
                                ...
                                <v-toolbar fixed dense flat>
                                    ...
                                    <v-toolbar-items>
                                        <v-btn
                                            icon
                                            large
                                            @click="newConfigModal = false; newConfigLoader = false"
                                        >
                                            <v-icon large>close</v-icon>
                                        </v-btn>
                                    </v-toolbar-items>
            ...
            <v-layout justify-space-around fill-height class="ml-5 pl-5 mt-4 mb-4">
                <v-flex xs3>
                    <v-btn color="primary" large flat @click="openPrintOptions">
                ...
                <v-flex xs3>
                    <v-dialog v-model="exportDialog" width="500">
                        <v-btn slot="activator" color="primary" large flat>
                            <v-icon>call_made</v-icon>&nbsp; EXPORT
                        </v-btn>
                        <v-card>
                            ...
                            <v-card-text>
                                ...
                                <v-btn
                                    ...
                                    color="primary"
                                    @click="copyPilot"
                                >Copy Pilot Data to Clipboard</v-btn>
                            </v-card-text>
                            <v-divider/>
                            <v-card-actions>
                ...
                <v-flex xs3>
                    <v-btn slot="activator" color="primary" large flat @click="clonePilot">
                        <v-icon>file_copy</v-icon>&nbsp; CLONE
                    </v-btn>
                </v-flex>

                <v-flex xs3>
                    <v-dialog v-model="deleteDialog" width="500">
                        <v-btn slot="activator" color="error" large flat>

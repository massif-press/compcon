<template>
  <v-container fluid>
    <v-layout>
      <v-flex xs2>
        <div class="sidebar">
          <ul class="menu" v-scroll-spy-active="{class: 'customActive'}" v-scroll-spy-link>
            <li v-for="s in statuses.Status" :key="'key_' + s.name" class="effect-text text-uppercase pt-1 pb-1">
              <a>{{s.name}}</a>
            </li>
            <li v-for="s in statuses.Condition" :key="'key_' + s.name" class="effect-text text-uppercase pt-1 pb-1">
              <a>{{s.name}}</a>
            </li>
          </ul>
        </div>
      ...
      <v-flex xs9>   
        <span class="display-1 text-uppercase font-weight-thin">STATUSES AND CONDITIONS</span>
        <br><br>

    ...
    <v-layout row>
            ...
            <v-flex class="mt-2 mb-2 mr-1 text-xs-right">
                <v-tooltip top>
                  <v-btn slot="activator" icon class="ma-0" @click="activateConfig()">
                    <v-icon :color="mech.IsActive ? 'teal accent-3' : 'grey lighten-1'">mdi-power</v-icon>
                  </v-btn>
                  <div class="text-xs-center">
                    <span><b :class="activeColorClass()"> {{mech.IsActive ? 'Active' : 'Inactive'}}</b>
                      <br><i>Click to {{mech.IsActive ? 'deactivate' : 'activate'}} Mech</i></span>
                  </div>
                </v-tooltip>
              <v-tooltip top>
      ...
      <lazy-dialog :model="deleteDialog" title="Delete Mech Configuration" acceptString="Delete"
        ...
        <template v-slot:modal-content>
          <v-card-text>Are you sure you want to delete {{mech.name}}? This action cannot be undone</v-card-text>
        </template>
      </lazy-dialog>
  ...
  export default Vue.extend({
    ...
    methods: {
      ...
      activeColorClass (): string {
        return this.mech.IsActive ? 'success--text text--lighten-2' : 'grey--text text--lighten-1' 
      },
      panelColor (): string {
        return this.mech.IsActive ? `rgba(4, 48, 114, 0.55)` : `rgba(0, 0, 0, 0.55)`
      },
      toConfigSheet () {
      ...
      activateConfig () {
        ...
        this.notify(`${this.mech.Name} ${this.mech.IsActive ? 'Activated' : 'Deactivated'}`)
      },    
      deleteConfig () {

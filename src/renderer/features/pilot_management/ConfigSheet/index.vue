<template>
  <div id="config-sheet" class="roster-content">
    ...
    <div v-else-if="config">
      <v-tooltip bottom>
        <v-toolbar slot="activator" dense :color="config.IsActive? 'warning' : 'grey darken-2'" class="ml-1">
            <v-divider/>
          <v-toolbar-title class="text-uppercase font-weight-light">
            <span style="letter-spacing: 15px; font-size: 1.75em"> Mech {{config.IsActive? 'Active' : 'Inactive'}}</span>
          </v-toolbar-title>
            <v-divider/>
              <v-toolbar-items class="hidden-sm-and-down">
                <v-btn large icon class="ma-0 ml-2" @click="activateConfig">
                  <v-icon large :color="config.IsActive ? 'yellow accent-2' : 'grey darken-3'">mdi-power</v-icon>
                </v-btn>
              </v-toolbar-items>
        </v-toolbar>
        <div class="text-xs-center">
          <span v-if="config.IsActive"><b>Active Mech</b><br>
      ...
      <v-container fluid class="pt-0">
        ...
        <v-layout align-end>
          ...
          <v-flex>
            <lazy-dialog :model="frameInfoModal" :title="`${config.Frame.Source} ${config.Frame.Name}`" hide-confirm @cancel="frameInfoModal = false">
          ...
          <v-flex xs7>
            ...
            <v-layout>
              <v-flex class="mr-3 ml-3 mt-1 mb-0">
                <v-alert type="warning" :value="config.RequiredLicenses.filter(x => x.missing).length" outline><b>WARNING: UNLICENSED COMPONENTS</b><br>
        ...
        <v-layout>
                  ...
                  <v-layout>
                    <v-flex shrink>
                      <span class="grey--text">STRUCTURE 
                        <b :style="`color: ${color.structure.dark}`">{{config.CurrentStructure || 0}}
                          <span v-if="config.IsActive">/{{config.MaxStructure}}</span>
                        </b>
                      </span>
                      <tick-bar :current="config.CurrentStructure || config.MaxStructure" :max="config.MaxStructure" large :color="color.structure.dark" bg-color="pink darken-4" empty-icon="mdi-hexagon-outline"  full-icon="cc-structure" config :readonly="!config.IsActive" @update="config.CurrentStructure = $event"/>
                    </v-flex>
                    <v-flex>
                      ...
                      <v-layout>
                        ...
                        <v-flex shrink>
                          <v-rating class="d-inline-flex" v-model="config.Armor" :length="config.Armor" readonly large dense full-icon="mdi-shield" :color="color.armor.dark"/>
                        </v-flex>
                      </v-layout>
                    </v-flex>
                  </v-layout>
                  ...
                  <v-layout>
                    <v-flex shrink>
                      <span class="grey--text">REACTOR STRESS 
                        <b :style="`color: ${color.stress.dark}`">{{config.CurrentStress || 0}}
                          <span v-if="config.IsActive">/{{config.MaxStress}}</span>
                        </b></span>
                      <tick-bar :current="config.CurrentStress || config.MaxStress" :max="config.MaxStress" large :color="color.stress.dark" bg-color="deep-orange darken-4" empty-icon="mdi-circle-outline" full-icon="cc-reactor-stress"  :readonly="!config.IsActive" @update="config.CurrentStress = $event"/>
                    </v-flex>
                    <v-flex>
                      <span class="grey--text">
                        <span v-if="config.IsActive">&nbsp;HEAT: <b :style="`color: ${color.heatcap.dark}`">{{config.CurrentHeat || 0}}</b> &emsp; &nbsp;</span>
                        HEAT CAPACITY <b :style="`color: ${color.heatcap.dark}`">{{config.HeatCapacity}}</b></span>
                      <tick-bar :current="config.CurrentHeat || 0" :max="config.HeatCapacity" large :color="color.heatcap.dark" bg-color="red darken-4" empty-icon="mdi-circle-outline" full-icon="mdi-circle" :readonly="!config.IsActive" @update="config.CurrentHeat = $event"/>
                    </v-flex>
                    <v-spacer />
                      <v-flex>
                      <span class="grey--text"> &nbsp;REPAIR CAPACITY <b :style="`color: ${color.repcap.dark}`">{{config.CurrentRepairs}}
                        <span v-if="config.IsActive">/{{config.RepairCapacity}}</span>
                      </b></span>
                      <tick-bar :current="config.CurrentRepairs || config.RepairCapacity" :max="config.RepairCapacity" large :color="color.repcap.dark" bg-color="grey darken-2" empty-icon="mdi-circle-outline" full-icon="control_point" :readonly="!config.IsActive" @update="config.CurrentRepairs = $event"/>
                    </v-flex>
                    <v-spacer />
                    <v-flex v-if="config.IsActive">
                      ...
                      <tick-bar :config_id="config.id" :current="config.CurrentCoreEnergy || 1" :max="1" large :color="color.corepower.dark" bg-color="grey darken-2" empty-icon="mdi-battery-10" full-icon="mdi-battery" @update="config.CurrentCoreEnergy = $event"/>
                    </v-flex>
                    <v-spacer />
                    <v-flex v-if="config.IsActive">
                      ...
                      <tick-bar :config_id="config.id" :current="config.CurrentOvercharge" :max="3" large :color="color.overcharge.dark" bg-color="grey darken-2" empty-icon="mdi-circle-outline" full-icon="mdi-alert-decagram" @update="config.CurrentOvercharge = $event"/>
                    </v-flex>
                  </v-layout>
                </v-flex>
              </v-layout>
              <v-layout justify-space-between>
                <statblock-item :attr="'Attack Bonus'" signed :val="config.AttackBonus" />
                <statblock-item :attr="'Tech Attack'" signed :val="config.TechAttack" />
                <statblock-item :attr="'Limited System Bonus'" signed :val="config.LimitedBonus" />
              </v-layout>
              <v-layout justify-space-between>
                ...
                <statblock-item :attr="'Sensor Range'" :val="config.SensorRange" />
          ...
          <v-layout>
            <v-flex class="m-2">
              <v-toolbar color="grey darken-2"><v-toolbar-title class="white--text">{{config.Frame.CoreSystem.Name}}</v-toolbar-title></v-toolbar>
              <v-card dark>
                <v-card-text v-if="config.Frame.CoreSystem.Ddescription" v-html="config.Frame.CoreSystem.Description" />
                <div v-if="config.Frame.CoreSystem.Passive">
                  <v-card-title class="subheading">Passive</v-card-title>
                  <v-card-text class="mt-0 pt-0 mb-0 pb-1"><p class="mb-1" v-html="config.Frame.CoreSystem.Passive" /></v-card-text>
                </div>
                <v-card-title class="title">{{config.Frame.CoreSystem.Active}}<span class="pt-2 ml-2 caption grey--text">(ACTIVE)</span></v-card-title>
                <v-card-text class="mt-0 pt-0 mb-0 pb-1"><p class="mb-1" v-html="config.Frame.CoreSystem.Effect" />
                <item-tag v-for="t in config.Frame.CoreSystem.Tags" :key="t.id" :tag-obj="t" /></v-card-text>
              </v-card>
            </v-flex>
          </v-layout>
            ...
            <v-card-text slot="modal-content">
              <v-layout>
                <v-flex class="mr-3 ml-3 mt-0 mb-0">
                  <v-alert type="error" :value="config.RequiredLicenses.filter(x => x.missing).length">

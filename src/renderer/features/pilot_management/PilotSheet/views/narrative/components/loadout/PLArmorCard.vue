<template>
  ...
  >
    ...
    >
      <span class="h2 heading text--text" style="line-height: 35px">{{ item.Name }}</span>
      ...
      <cc-selector-table :items="getArmor()" :headers="headers" @equip="equip($event)">
        <div v-if="item">
          <span class="overline">
            ...
            >[ PILOT MATERIEL REGISTRATION VERIFIED ]</span>
          </span>
          <br />
        ...
        <div v-else>
          <span
            class="overline"
          >GMS ARMORY EQUIPMENT AUTHORIZATION: PILOT/PERSONAL ARMOR::TI - TVII-A</span>
          <br />
    ...
    <v-card slot="detail">test</v-card>
  </pl-card-base>
</template>

<script lang="ts">
import Vue from 'vue'
import PlCardBase from './PLCardBase.vue'

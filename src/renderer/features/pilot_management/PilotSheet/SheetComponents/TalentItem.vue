<template>
  <v-expansion-panel-content>
    ...
    <div v-if="!talent.err">
      <v-card >
        <v-card-title><p class="fluff-text mb-0 pb-0 pt-0" v-html="talent.description" /></v-card-title>
      </v-card>
      <v-card v-for="(r, index) in talent.Ranks" :key="r.name + index + rank()" 
        :color="isLocked(index) ? 'grey lighten-4' : ''">
        <v-card-title primary-title class="pb-0 pt-0 mb-0">
          <div style="width:100%">
            <div class="title">
              <v-icon class="mt-2" :color="isLocked(index) ? 'grey darken-2' : 'primary'" 

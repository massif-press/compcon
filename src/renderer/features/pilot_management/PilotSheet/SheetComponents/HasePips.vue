...
export default Vue.extend({
  ...
  methods: {
    tooltip(): string {
      switch (this.title) {
        case 'hull':
          return `MECH HP <b>+${this.attr * 2}</b><br>REPAIR CAPACITY <b>+${Math.floor(
            this.attr / 2
          )}</b>`
        case 'agility':
          return `EVASION <b>+${this.attr}</b><br>SPEED<b>+${Math.floor(this.attr / 2)}</b>`
        case 'systems':
          return `E-DEFENSE <b>+${this.attr}</b><br>TECH ATTACK <b>+${
            this.attr
          }</b><br>SP <b>+${Math.floor(this.attr / 2)}</b>`
        case 'engineering':
          return `HEAT CAPACITY <b>+${this.attr}</b><br>LIMITED SYSTEMS BONUS <b>+${Math.floor(
            this.attr / 2
          )}</b>`
        default:
          return ''

<template>
  <v-expansion-panel-content>
    ...
    <v-card>
      <v-row v-if="selectable && available" class="text-center">
        <v-col v-if="!pilotRank" class="ma-2 ml-5 mr-5">
          <v-btn @click="add" color="primary" large block>unlock rank I</v-btn>
        </v-col>
        <v-col v-else-if="pilotRank === 1" class="ma-2 ml-5 mr-5">
          <v-btn @click="add" color="primary" large block>unlock rank II</v-btn>
        </v-col>
        <v-col v-else-if="pilotRank === 2" class="ma-2 ml-5 mr-5">
      ...
      <v-row v-if="selectable" class="text-center">
        <v-col v-if="pilotRank === 1" class="ma-2">
          <v-btn @click="remove" flat color="warning" block>
            remove rank I
          </v-btn>
        </v-col>
        <v-col v-else-if="pilotRank === 2" class="ma-2">
          <v-btn @click="remove" flat color="warning" block>
            remove rank II
          </v-btn>
        </v-col>
        <v-col v-else-if="pilotRank === 3" class="ma-2">
          <v-btn @click="remove" flat color="warning" block>
            remove rank III
          </v-btn>
        </v-col>

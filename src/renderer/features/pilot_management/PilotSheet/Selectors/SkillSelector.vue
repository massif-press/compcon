        ...
        <v-flex xs12>
          <div v-for="pSkill in pilot.Skills" :key="`summary_${pSkill.Skill.ID}`">
            <v-layout v-if="pSkill.err">
              ...
              <v-flex shrink>
                <v-btn icon flat color="error" @click="subtract(pSkill)"><v-icon>delete</v-icon></v-btn>
              </v-flex>
            </v-layout>
            <v-layout v-else>
              <v-flex xs12>
                <v-chip dark color="primary" outline small>+<b>{{pSkill.Bonus}}</b></v-chip>
  ...
  export default Vue.extend({
    ...
    computed: {
      points () {
        var vm = this as any
        return {
          pointsCurrent: (vm.pilot.Skills.reduce((a: number, b: PilotSkill) => +a + +b.Rank, 0)),

<template>
  <v-container fluid>
    <v-layout>
        ...
        <div class="sidebar">
          <ul class="menu" v-scroll-spy-active="{class: 'customActive'}" v-scroll-spy-link>
            <li v-for="b in backgrounds" :key="'key_' + b.ID" class="minor-title pt-1 pb-1">
              <a :class="pilot.Background.Name === b.Name ? 'highlight' : ''">{{b.Name}}</a>
            </li>
            <li class="minor-title pt-1 pb-1">
              <a>Custom Background</a>
            </li>
          </ul>
        </div>
      </v-flex>
      ...
      <v-flex xs9>
        <div style="position:relative" v-scroll-spy="{offset: 60, time:0}" >

...
class Statblock {
  public static Generate(pilot: Pilot | null, mech: Mech | null) {
    function linebreak(i: number, length: number) {
      if (i > 0 && (i + 1) % 2 === 0 && i + 1 !== length) { return '\n  ' }
      else if (i + 1 < length) { return ', ' }
      else { return '\n' }
    }
...
`» ${pilot.Callsign.toUpperCase()} «
  ...
  GRIT:${pilot.Grit} // H:${pilot.MechSkills.Hull} A:${pilot.MechSkills.Agi} S:${pilot.MechSkills.Sys} E:${pilot.MechSkills.Eng}\n`
      ...
      for (let i = 0; i < pilot.Licenses.length; i++) {
        ...
        output += `${l.License.Source} ${l.License.Name} ${l.Rank}${linebreak(i, pilot.Licenses.length)}`
      }
      ...
      if (loadout) {
        ...
        for (let i = 0; i < loadout.Items.length; i++) {
          if (loadout.Items[i]) {
            ...
            if (i > 0 && (i + 1) % 2 === 0 && i + 1 !== loadout.Items.length) { output += '\n  ' }
            else if (i + 1 < loadout.Items.length) { output += ', ' }
          }
        }
      }
    }
    ...
    if (mech) {
      ...
      if (!pilot) output += `H:${mech.Hull} A:${mech.Agi} S:${mech.Sys} E:${mech.Eng} SIZE:${mech.Size}\n `

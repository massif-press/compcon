<template>
  <v-flex shrink v-show="!tagObj.Hidden">
    <v-tooltip top offset-y>
      ...
      <v-card-text
        v-if="tagObj.err"
      >Tag data not found. This is likely due to a missing content pack, or malformed item data.</v-card-text>
      <template v-else>
        <v-card-title
          class="minor-title pa-0 ma-0"
        >{{ pilot ? tagObj.Name(pilot.LimitedBonus) : tagObj.Name() }}</v-card-title>

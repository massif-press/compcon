<template>
    <v-flex shrink>
      <v-tooltip top offset-y>
        <v-chip class="px-1 py-2" slot="activator" :color="tagObj.err? 'deep-orange' : 'primary'" dark small>
          <v-avatar>
            <v-icon small v-if="tagObj.err">label_off</v-icon>
            <v-icon small v-else>label</v-icon>
          </v-avatar>
          <span v-if="tagObj.err">MISSING DATA</span>
          <span v-else>{{ pilot ? tagObj.Name(pilot.LimitedBonus) : tagObj.Name() }}</span>
        </v-chip>
        ...
        <template v-else>
          ...
          <v-card-text v-if="tagObj.ID === 'limited' && pilot && pilot.LimitedBonus" class="pb-2 mb-2 pt-0 mt-0">{{ tagObj.Description(pilot.LimitedBonus) }}
            <br>
            <em v-if="pilot.LimitedBonus">(Includes +{{pilot.LimitedBonus}} Limited Systems bonus)</em>
          </v-card-text>

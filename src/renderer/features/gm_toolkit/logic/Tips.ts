...
export const NPCTips = (npc: NPC): Tip[] => {
  ...
  if (templates.includes('exotic')) {
    const exoticSystemsCount = systems.filter(sys => sys.class === 'exotic').length
    output.push({
      id: 'exotic',
  ...
  if (templates.includes('veteran')) {
    ...
    const veteranTraitCount = systems.filter(sys => sys.class === 'veteran').length
    output.push({
      id: 'veteran',
  ...
  if (templates.includes('ultra')) {
    const ultraSystemsCount = systems.filter(sys => sys.class === 'ultra' && !sys.base).length
    output.push({
      id: 'ultra',

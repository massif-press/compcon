<template>
  <v-container fluid>
          ...
          <template v-slot:extra-icons>
            <v-menu bottom left>
              ...
              <v-list class="more-list">
                <v-list-tile v-ripple :to="`/npc-designer/edit/${npc.id}`">
    ...
    <v-dialog v-if="deletingNPCID !== null" v-model="deleteDialog" persistent max-width="290">
      <v-card>
        <v-card-title class="headline">Delete {{ deletingNPC.name }}?</v-card-title>
        <v-card-text>This cannot be undone.</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="secondary" flat @click="deleteNPC(deletingNPCID)">OK</v-btn>
          <v-btn
            color="secondary"
            flat
            @click="

...
export default Vue.extend({
  ...
  computed: {
    isMultiline(): boolean {
      return Boolean(
        (this.system as NPCSystem.NonWeapon).action ||
          (this.system as NPCSystem.NonWeapon).tech_roll ||
          (this.system as NPCSystem.NonWeapon).recharge ||
          this.system.type === 'weapon'
      )
    },
    shortName(): string {
    ...
    rollString(): string {
      ...
      const { flat, accdiff } =
        (this.system as NPCSystem.Weapon).weapon_roll ||
        (this.system as NPCSystem.NonWeapon).tech_roll
      const { tier } = this.npc
      let output = ''
      const flatTotal = flat ? (flat.pertier ? flat.val * (tier + 1) : flat.val) : 0
      if (flatTotal) {
      ...
      output += ' vs '
      output +=
        (this.system as NPCSystem.Weapon).smart || (this.system as NPCSystem.NonWeapon).tech_roll
          ? 'EDEF'
          : 'EV'
      return output
    },
